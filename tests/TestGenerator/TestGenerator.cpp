...
void test_generator::addBlock(const VarNote::CachedBlock& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(VarNote::BlockTemplate& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(VarNote::BlockTemplate& blk, const VarNote::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<VarNote::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(VarNote::BlockTemplate& blk, const VarNote::BlockTemplate& blkPrev,
...
bool test_generator::constructBlockManuallyTx(VarNote::BlockTemplate& blk, const VarNote::BlockTemplate& prevBlock,
                                              const VarNote::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(VarNote::BlockTemplate& blk, const VarNote::BlockTemplate& blkPrev,
...
void fillNonce(VarNote::BlockTemplate& blk, const Difficulty& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxManually(const VarNote::Currency& currency, uint8_t blockMajorVersion, uint32_t height, uint64_t alreadyGeneratedCoins,
...
bool constructMinerTxBySize(const VarNote::Currency& currency, VarNote::Transaction& baseTransaction, uint8_t blockMajorVersion, uint32_t height,
  ...
  if (!currency.constructMinerTx(blockMajorVersion, height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, VarNote::BinaryArray(), 1)) {
    return false;
  }

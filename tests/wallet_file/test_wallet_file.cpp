...
// no password
// created by simplewallet from varcoin-2.1.2, several tx received and sent
...
static void test_body(const varcoin::Currency &currency, const std::string &path, const std::string &password,
    const std::vector<std::string> &addresses, bool view_only) {
	varcoin::Wallet wallet("test_wallet_file.tmp", password);
	...
	for (auto &&a : addresses) {
		varcoin::AccountPublicAddress address;
...
static void test_single_file(const varcoin::Currency &currency, const std::string &path, const std::string &password,
    const std::vector<std::string> &addresses, bool view_only) {
	...
	test_body(currency, "test_wallet_file.tmp", password, addresses, view_only);
	{
		...
		if (si != varcoin::Wallet::wallet_file_size(addresses.size()))
...
void test_wallet_file(const std::string &path_prefix) {
	varcoin::Currency currency(false);

...
bool gen_block_ts_in_future::generate(std::vector<test_event_entry>& events) const
{
  ...
  block blk_1;
  generator.construct_block_manually(blk_1, blk_0, miner_account, test_generator::bf_timestamp, 0, 0, time(NULL) + 60*60 + VarNote_BLOCK_FUTURE_TIME_LIMIT);
  events.push_back(blk_1);
...
bool gen_block_unlock_time_is_timestamp_in_future::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_MINER_TX_MANUALLY(miner_tx, blk_0);
  miner_tx.unlock_time = blk_0.timestamp + 3 * VarNote_MINED_MONEY_UNLOCK_WINDOW * DIFFICULTY_BLOCKS_ESTIMATE_TIMESPAN;
...
bool gen_block_is_too_big::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_MINER_TX_MANUALLY(miner_tx, blk_0);
  static const size_t tx_out_count = VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE / 2;
...
bool gen_block_invalid_binary_format::generate(std::vector<test_event_entry>& events) const
{
  ...
  assert(VarNote_MINED_MONEY_UNLOCK_WINDOW < DIFFICULTY_WINDOW);
  for (size_t i = 0; i < VarNote_MINED_MONEY_UNLOCK_WINDOW; ++i)
  {

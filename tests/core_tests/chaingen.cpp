...
uint64_t test_generator::get_already_generated_coins(const VarNote::block& blk) const
{
...
void test_generator::add_block(const VarNote::block& blk, size_t tsx_size, std::vector<size_t>& block_sizes, uint64_t already_generated_coins)
{
...
bool test_generator::construct_block(VarNote::block& blk, uint64_t height, const crypto::hash& prev_id,
...
bool test_generator::construct_block(VarNote::block& blk, const VarNote::account_base& miner_acc, uint64_t timestamp)
{
...
bool test_generator::construct_block(VarNote::block& blk, const VarNote::block& blk_prev,
                                     const VarNote::account_base& miner_acc,
                                     const std::list<VarNote::transaction>& tx_list/* = std::list<VarNote::transaction>()*/)
{
  ...
  std::vector<size_t> block_sizes;
  get_last_n_block_sizes(block_sizes, prev_id, VarNote_REWARD_BLOCKS_WINDOW);
                                              ...
                                              size_t txs_sizes/* = 0*/)
{
  ...
  std::vector<size_t> block_sizes;
  get_last_n_block_sizes(block_sizes, get_block_hash(prev_block), VarNote_REWARD_BLOCKS_WINDOW);
  ...
  else
  {
    ...
    // TODO: This will work, until size of constructed block is less then VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE
...
bool test_generator::construct_block_manually_tx(VarNote::block& blk, const VarNote::block& prev_block,
                                                 const VarNote::account_base& miner_acc,
...
struct output_index {
    const VarNote::txout_target_v out;
    uint64_t amount;
    ...
    size_t idx;
    bool spent;
    const VarNote::block *p_blk;
    const VarNote::transaction *p_tx;

    output_index(const VarNote::txout_target_v &_out, uint64_t _a, size_t _h, size_t tno, size_t ono, const VarNote::block *_pb, const VarNote::transaction *_pt)
...
bool init_output_indices(map_output_idx_t& outs, std::map<uint64_t, std::vector<size_t> >& outs_mine, const std::vector<VarNote::block>& blockchain, const map_hash2tx_t& mtx, const VarNote::account_base& from) {
...
bool init_spent_output_indices(map_output_idx_t& outs, map_output_t& outs_mine, const std::vector<VarNote::block>& blockchain, const map_hash2tx_t& mtx, const VarNote::account_base& from) {
...
bool fill_tx_sources(std::vector<tx_source_entry>& sources, const std::vector<test_event_entry>& events,
                     const block& blk_head, const VarNote::account_base& from, uint64_t amount, size_t nmix)
{
    map_output_idx_t outs;
    map_output_t outs_mine;

    std::vector<VarNote::block> blockchain;
    map_hash2tx_t mtx;
...
bool fill_tx_destination(tx_destination_entry &de, const VarNote::account_base &to, uint64_t amount) {
...
void fill_nonce(VarNote::block& blk, const difficulty_type& diffic, uint64_t height)
{
  blk.nonce = 0;
  crypto::cn_context context;
                                 ...
                                 keypair* p_txkey/* = 0*/)
{
  ...
  // This will work, until size of constructed block is less then VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE
  uint64_t block_reward;
  ...
  tx.unlock_time = height + VarNote_MINED_MONEY_UNLOCK_WINDOW;

  return true;
}

bool construct_tx_to_key(const std::vector<test_event_entry>& events, VarNote::transaction& tx, const block& blk_head,
                         const VarNote::account_base& from, const VarNote::account_base& to, uint64_t amount,
...
bool test_chain_unit_base::verify(const std::string& cb_name, VarNote::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{

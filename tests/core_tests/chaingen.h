...
struct serialized_object
{
  ...
  serialized_object(const VarNote::blobdata& a_data)
    : data(a_data)
  {
  }

  VarNote::blobdata data;
  BEGIN_SERIALIZE_OBJECT()
    FIELD(data)
    END_SERIALIZE()
...
typedef serialized_object<VarNote::block> serialized_block;
...
VARIANT_TAG(binary_archive, VarNote::account_base, 0xcc);
VARIANT_TAG(binary_archive, serialized_block, 0xcd);
VARIANT_TAG(binary_archive, serialized_transaction, 0xce);
VARIANT_TAG(binary_archive, event_visitor_settings, 0xcf);

typedef boost::variant<VarNote::block, VarNote::transaction, VarNote::account_base, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
typedef std::unordered_map<crypto::hash, const VarNote::transaction*> map_hash2tx_t;

class test_chain_unit_base
{
public:
  typedef boost::function<bool (VarNote::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
  typedef std::map<std::string, verify_callback> callbacks_map;

  void register_callback(const std::string& cb_name, verify_callback cb);
  bool verify(const std::string& cb_name, VarNote::core& c, size_t ev_index, const std::vector<test_event_entry> &events);
private:
...
public:
  ...
  uint64_t get_already_generated_coins(const VarNote::block& blk) const;

  void add_block(const VarNote::block& blk, size_t tsx_size, std::vector<size_t>& block_sizes, uint64_t already_generated_coins);
  bool construct_block(VarNote::block& blk, uint64_t height, const crypto::hash& prev_id,
    const VarNote::account_base& miner_acc, uint64_t timestamp, uint64_t already_generated_coins,
    std::vector<size_t>& block_sizes, const std::list<VarNote::transaction>& tx_list);
  bool construct_block(VarNote::block& blk, const VarNote::account_base& miner_acc, uint64_t timestamp);
  bool construct_block(VarNote::block& blk, const VarNote::block& blk_prev, const VarNote::account_base& miner_acc,
    const std::list<VarNote::transaction>& tx_list = std::list<VarNote::transaction>());

  bool construct_block_manually(VarNote::block& blk, const VarNote::block& prev_block,
    const VarNote::account_base& miner_acc, int actual_params = bf_none, uint8_t major_ver = 0,
    uint8_t minor_ver = 0, uint64_t timestamp = 0, const crypto::hash& prev_id = crypto::hash(),
    const VarNote::difficulty_type& diffic = 1, const VarNote::transaction& miner_tx = VarNote::transaction(),
    const std::vector<crypto::hash>& tx_hashes = std::vector<crypto::hash>(), size_t txs_sizes = 0);
  bool construct_block_manually_tx(VarNote::block& blk, const VarNote::block& prev_block,
    const VarNote::account_base& miner_acc, const std::vector<crypto::hash>& tx_hashes, size_t txs_size);

private:
  std::unordered_map<crypto::hash, block_info> m_blocks_info;
};

inline VarNote::difficulty_type get_test_difficulty() {return 1;}
void fill_nonce(VarNote::block& blk, const VarNote::difficulty_type& diffic, uint64_t height);

bool construct_miner_tx_manually(size_t height, uint64_t already_generated_coins,
                                 const VarNote::account_public_address& miner_address, VarNote::transaction& tx,
                                 uint64_t fee, VarNote::keypair* p_txkey = 0);
bool construct_tx_to_key(const std::vector<test_event_entry>& events, VarNote::transaction& tx,
                         const VarNote::block& blk_head, const VarNote::account_base& from, const VarNote::account_base& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
VarNote::transaction construct_tx_with_fee(std::vector<test_event_entry>& events, const VarNote::block& blk_head,
                                            const VarNote::account_base& acc_from, const VarNote::account_base& acc_to,
                                            uint64_t amount, uint64_t fee);

void get_confirmed_txs(const std::vector<VarNote::block>& blockchain, const map_hash2tx_t& mtx, map_hash2tx_t& confirmed_txs);
bool find_block_chain(const std::vector<test_event_entry>& events, std::vector<VarNote::block>& blockchain, map_hash2tx_t& mtx, const crypto::hash& head);
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const VarNote::block& blk_head,
                                      const VarNote::account_base& from, const VarNote::account_base& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<VarNote::tx_source_entry>& sources,
                                      std::vector<VarNote::tx_destination_entry>& destinations);
uint64_t get_balance(const VarNote::account_base& addr, const std::vector<VarNote::block>& blockchain, const map_hash2tx_t& mtx);

//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_tx_verification_context(const VarNote::tx_verification_context& tvc, bool tx_added, size_t event_index, const VarNote::transaction& tx, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_tx_verification_context(const VarNote::tx_verification_context& tvc, bool tx_added, size_t /*event_index*/, const VarNote::transaction& /*tx*/, t_test_class&, long)
{
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_tx_verification_context(const VarNote::tx_verification_context& tvc, bool tx_added, size_t event_index, const VarNote::transaction& tx, t_test_class& validator)
{
  // SFINAE in action
...
//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_block_verification_context(const VarNote::block_verification_context& bvc, size_t event_index, const VarNote::block& blk, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_block_verification_context(const VarNote::block_verification_context& bvc, size_t /*event_index*/, const VarNote::block& /*blk*/, t_test_class&, long)
{
  // Default block verification context check
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_block_verification_context(const VarNote::block_verification_context& bvc, size_t event_index, const VarNote::block& blk, t_test_class& validator)
{
  // SFINAE in action
...
public:
  ...
  bool operator()(const VarNote::transaction& tx) const
  {
    ...
    VarNote::tx_verification_context tvc = AUTO_VAL_INIT(tvc);
  ...
  bool operator()(const VarNote::block& b) const
  {
    log_event("VarNote::block");

    VarNote::block_verification_context bvc = AUTO_VAL_INIT(bvc);
  ...
  bool operator()(const VarNote::account_base& ab) const
  {
...
//--------------------------------------------------------------------------
template<class t_test_class>
inline bool replay_events_through_core(VarNote::core& cr, const std::vector<test_event_entry>& events, t_test_class& validator)
{
  TRY_ENTRY();

  //init core here

  CHECK_AND_ASSERT_MES(typeid(VarNote::block) == events[0].type(), false, "First event must be genesis block creation");
  cr.set_genesis_block(boost::get<VarNote::block>(events[0]));
...
inline bool do_replay_events(std::vector<test_event_entry>& events)
{
  ...
  VarNote::core::init_options(desc);
  ...
  VarNote::VarNote_protocol_stub pr; //TODO: stub only for this kind of test, make real validation of relayed objects
  VarNote::core c(&pr);
...
#define REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, COUNT)           \
  VarNote::block BLK_NAME;                                                           \
  {                                                                                   \
    VarNote::block blk_last = PREV_BLOCK;                                            \
...
#define REWIND_BLOCKS(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC) REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, VarNote_MINED_MONEY_UNLOCK_WINDOW)

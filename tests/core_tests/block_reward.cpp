...
namespace
{
  ...
  bool construct_max_size_block(test_generator& generator, block& blk, const block& blk_prev, const account_base& miner_account,
    size_t median_block_count = VarNote_REWARD_BLOCKS_WINDOW)
  {
    ...
    median = std::max(median, static_cast<size_t>(VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE));
...
bool gen_block_reward::generate(std::vector<test_event_entry>& events) const
{
  ...
  block blk_0r;
  if (!rewind_blocks(events, generator, blk_0r, blk_0, miner_account, VarNote_REWARD_BLOCKS_WINDOW))
    return false;

  // Test: block reward is calculated using median of the latest VarNote_REWARD_BLOCKS_WINDOW blocks
  DO_CALLBACK(events, "mark_invalid_block");
  block blk_1_bad_1;
  if (!construct_max_size_block(generator, blk_1_bad_1, blk_0r, miner_account, VarNote_REWARD_BLOCKS_WINDOW + 1))
    return false;
  events.push_back(blk_1_bad_1);

  DO_CALLBACK(events, "mark_invalid_block");
  block blk_1_bad_2;
  if (!construct_max_size_block(generator, blk_1_bad_2, blk_0r, miner_account, VarNote_REWARD_BLOCKS_WINDOW - 1))
    return false;
  ...
  block blk_5r;
  if (!rewind_blocks(events, generator, blk_5r, blk_5, miner_account, VarNote_MINED_MONEY_UNLOCK_WINDOW))
    return false;
  ...
  // Test: block reward == transactions fee
  {
    ...
    std::vector<size_t> block_sizes;
    generator.get_last_n_block_sizes(block_sizes, get_block_hash(blk_7), VarNote_REWARD_BLOCKS_WINDOW);
    size_t median = misc_utils::median(block_sizes);
...
bool gen_block_reward::check_block_verification_context(const VarNote::block_verification_context& bvc, size_t event_idx, const VarNote::block& /*blk*/)
{
...
bool gen_block_reward::mark_invalid_block(VarNote::core& /*c*/, size_t ev_index, const std::vector<test_event_entry>& /*events*/)
{
  m_invalid_block_index = ev_index + 1;
  return true;
}

bool gen_block_reward::mark_checked_block(VarNote::core& /*c*/, size_t ev_index, const std::vector<test_event_entry>& /*events*/)
{
  m_checked_blocks_indices.push_back(ev_index - 1);
  return true;
}

bool gen_block_reward::check_block_rewards(VarNote::core& /*c*/, size_t /*ev_index*/, const std::vector<test_event_entry>& events)
{

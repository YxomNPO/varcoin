...
//-----------------------------------------------------------------------------------------------------
bool gen_chain_switch_1::check_split_not_switched(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  CHECK_EQ(5 + 2 * VarNote_MINED_MONEY_UNLOCK_WINDOW, blocks.size());
  CHECK_TEST_CONDITION(blocks.back() == boost::get<block>(events[20 + 2 * VarNote_MINED_MONEY_UNLOCK_WINDOW]));  // blk_4
...
//-----------------------------------------------------------------------------------------------------
bool gen_chain_switch_1::check_split_switched(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  bool r = c.get_blocks(0, 10000, blocks);
  CHECK_TEST_CONDITION(r);
  CHECK_EQ(6 + 2 * VarNote_MINED_MONEY_UNLOCK_WINDOW, blocks.size());
  auto it = blocks.end();
  --it; --it; --it;
  CHECK_TEST_CONDITION(std::equal(blocks.begin(), it, m_chain_1.begin()));
  CHECK_TEST_CONDITION(blocks.back() == boost::get<block>(events[24 + 2 * VarNote_MINED_MONEY_UNLOCK_WINDOW]));  // blk_7

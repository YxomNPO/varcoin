...
namespace
{
  ...
  void append_tx_source_entry(std::vector<VarNote::tx_source_entry>& sources, const transaction& tx, size_t out_idx)
  {
    VarNote::tx_source_entry se;
    se.amount = tx.vout[out_idx].amount;
    se.outputs.push_back(std::make_pair(0, boost::get<VarNote::txout_to_key>(tx.vout[out_idx].target).key));
    se.real_output = 0;
...
bool gen_uint_overflow_base::check_tx_verification_context(const VarNote::tx_verification_context& tvc, bool tx_added, size_t event_idx, const VarNote::transaction& /*tx*/)
{
  return m_last_valid_block_event_idx < event_idx ? !tx_added && tvc.m_verifivation_failed : tx_added && !tvc.m_verifivation_failed;
}

bool gen_uint_overflow_base::check_block_verification_context(const VarNote::block_verification_context& bvc, size_t event_idx, const VarNote::block& /*block*/)
{
...
bool gen_uint_overflow_base::mark_last_valid_block(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
...
bool gen_uint_overflow_1::generate(std::vector<test_event_entry>& events) const
{
  ...
  std::list<VarNote::transaction> txs_1;

...
BOOST_CLASS_VERSION(nodetool::node_server<VarNote::t_VarNote_protocol_handler<tests::proxy_core> >, 1);
...
int main(int argc, char* argv[])
{
  ...
  nodetool::node_server<VarNote::t_VarNote_protocol_handler<tests::proxy_core> >::init_options(desc);
  ...
  VarNote::t_VarNote_protocol_handler<tests::proxy_core> cprotocol(pr_core, NULL);
  nodetool::node_server<VarNote::t_VarNote_protocol_handler<tests::proxy_core> > p2psrv(cprotocol);
  cprotocol.set_p2p_endpoint(&p2psrv);
  //pr_core.set_VarNote_protocol(&cprotocol);
  ...
  LOG_PRINT_L0("Initializing VarNote protocol...");
  res = cprotocol.init(vm);
  CHECK_AND_ASSERT_MES(res, 1, "Failed to initialize VarNote protocol.");
  LOG_PRINT_L0("VarNote protocol initialized OK");
  ...
  pr_core.deinit();
  LOG_PRINT_L0("Deinitializing VarNote_protocol...");
  cprotocol.deinit();
  LOG_PRINT_L0("Deinitializing p2p...");
  p2psrv.deinit();


  //pr_core.set_VarNote_protocol(NULL);
...
/*
string tx2str(const VarNote::transaction& tx, const VarNote::hash256& tx_hash, const VarNote::hash256& tx_prefix_hash, const VarNote::blobdata& blob) {
    stringstream ss;
...
bool tests::proxy_core::handle_incoming_tx(const VarNote::blobdata& tx_blob, VarNote::tx_verification_context& tvc, bool keeped_by_block) {
    if (!keeped_by_block)
        return true;
...
bool tests::proxy_core::handle_incoming_block(const VarNote::blobdata& block_blob, VarNote::block_verification_context& bvc, bool update_miner_blocktemplate) {
...
void tests::proxy_core::build_short_history(std::list<crypto::hash> &m_history, const crypto::hash &m_start) {
    ...
    do {
        ...
        while (m_hash2blkidx.end() != cit && VarNote::null_hash != cit->second.blk.prev_id && n > 0) {
            n--;
...
bool tests::proxy_core::add_block(const crypto::hash &_id, const crypto::hash &_longhash, const VarNote::block &_blk, const VarNote::blobdata &_blob) {
    size_t height = 0;

    if (VarNote::null_hash != _blk.prev_id) {

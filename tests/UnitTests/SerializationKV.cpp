...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = VarNote::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(VarNote::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = VarNote::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(VarNote::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}

...
#include <VarNoteCore/DatabaseBlockchainCache.h>
#include "VarNoteCore/VarNoteTools.h"
#include "VarNoteCore/TransactionValidatiorState.h"
#include "DataBaseMock.h"
#include <VarNoteCore/DBUtils.h>
...
TEST_F(DatabaseBlockchainCacheTests, RawBlocksWithTxsSerialization) {
  ...
  std::vector<VarNote::BlockTemplate> blocks = generator.getBlockchainCopy();
  ASSERT_LT(0, blocks.size());

  BlockTemplate lastBlock = blocks.back();
  ASSERT_EQ(TXS_COUNT, lastBlock.transactionHashes.size());

  VarNote::RawBlock rawBlock{ toBinaryArray(lastBlock), {} };
  ...
  std::string serializedRawBlock = VarNote::DB::serialize(rawBlock, SERIALIZATION_NAME);
  VarNote::RawBlock deserializedRawBlock;
  VarNote::DB::deserialize(serializedRawBlock, deserializedRawBlock, SERIALIZATION_NAME);

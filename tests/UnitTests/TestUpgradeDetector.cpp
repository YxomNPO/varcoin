...
namespace {
  using VarNote::BLOCK_MAJOR_VERSION_1;
  using VarNote::BLOCK_MAJOR_VERSION_2;
  using VarNote::BLOCK_MINOR_VERSION_0;
  using VarNote::BLOCK_MINOR_VERSION_1;

  struct BlockEx {
    VarNote::BlockTemplate bl;
  };

  typedef std::vector<BlockEx> BlockVector;
  typedef VarNote::BasicUpgradeDetector<BlockVector> UpgradeDetector;

  class UpgradeTest : public ::testing::Test {
  public:

    VarNote::Currency createCurrency(uint32_t upgradeHeight = UpgradeDetector::UNDEF_HEIGHT) {
      VarNote::CurrencyBuilder currencyBuilder(logger);
      ...
      currencyBuilder.upgradeHeightV3(VarNote::UpgradeDetectorBase::UNDEF_HEIGHT);
  ...
  TEST_F(UpgradeDetector_voting_init, handlesEmptyBlockchain) {
    VarNote::Currency currency = createCurrency();
    BlockVector blocks;
  ...
  TEST_F(UpgradeDetector_upgradeHeight_init, handlesEmptyBlockchain) {
    ...
    VarNote::Currency currency = createCurrency(upgradeHeight);
    BlockVector blocks;
    UpgradeDetector upgradeDetector(currency, blocks, BLOCK_MAJOR_VERSION_2, logger);
    ASSERT_TRUE(upgradeDetector.init());
    ASSERT_EQ(upgradeDetector.upgradeHeight(), upgradeHeight);

...
std::vector<VarNote::Block> TestBlockchainGenerator::getBlockchainCopy() {
  std::unique_lock<std::mutex> lock(m_mutex);

  std::vector<VarNote::Block> blockchain(m_blockchain);
  return blockchain;
}
...
void TestBlockchainGenerator::addGenesisBlock() {
  ...
  m_timestampIndex.add(m_currency.genesisBlock().timestamp, VarNote::get_block_hash(m_currency.genesisBlock()));
...
void TestBlockchainGenerator::addMiningBlock() {
  ...
  m_timestampIndex.add(block.timestamp, VarNote::get_block_hash(block));
  ...
  std::vector<VarNote::Transaction>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids)
{
  ...
  if (known_block_id != VarNote::get_block_hash(m_blockchain.back())) {
    is_bc_actual = false;
    return;
  }
...
bool TestBlockchainGenerator::getPoolTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Crypto::Hash>& hashes, uint64_t& transactionsNumberWithinTimestamps) {
  ...
  for (auto i : m_txPool) {
    ...
    hashes.push_back(VarNote::getObjectHash(i.second));
    ++c;
  }
  return true;
}

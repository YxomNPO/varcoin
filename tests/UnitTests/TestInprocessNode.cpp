...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  std::vector<VarNote::Block> expectedBlocks;
  ...
  for (const boost::tuple<boost::tuple<size_t, VarNote::Block>, std::vector<VarNote::BlockDetails>>& sameHeight : range) {
    ...
    for (const VarNote::BlockDetails& block : sameHeight.get<1>()) {
      ...
      Crypto::Hash expectedCryptoHash = VarNote::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHeightFail) {
  ...
  for (const VarNote::Block& block : generator.getBlockchain()) {
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    expectedBlocks.push_back(*iter);
    blockHashes.push_back(VarNote::get_block_hash(*iter));
    coreStub.addBlock(*iter);
  }
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<Crypto::Hash, VarNote::Block>, VarNote::BlockDetails>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = VarNote::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i) {
    ...
    expectedTransactions.push_back(std::make_tuple(tx, VarNote::get_block_hash(generator.getBlockchain().back()), boost::get<VarNote::BaseInput>(generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
  }
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp) {
  ...
  VarNote::InProcessNode newNode(core, protocolQueryStub);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    auto txptr = VarNote::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.insert(VarNote::getObjectHash(tx));
    VarNote::tx_verification_context tvc = boost::value_initialized<tx_verification_context>();
    bool keptByBlock = false;
    coreStub.handleIncomingTransaction(tx, VarNote::getObjectHash(tx), VarNote::getObjectBinarySize(tx), tvc, keptByBlock);

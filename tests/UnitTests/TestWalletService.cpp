...
struct IWalletBaseStub : public VarNote::IWallet, public VarNote::IFusionManager {
...
protected:
  ...
  std::unique_ptr<WalletService> createWalletService(VarNote::IWallet& wallet, VarNote::IFusionManager& fusionManager);
...
std::unique_ptr<WalletService> WalletServiceTest::createWalletService(VarNote::IWallet& wallet, VarNote::IFusionManager& fusionManager) {
...
TEST_F(WalletServiceTest_createAddress, invalidSecretKey) {
  ...
  ASSERT_EQ(make_error_code(VarNote::error::WalletServiceErrorCode::WRONG_KEY_FORMAT), ec);
}
...
TEST_F(WalletServiceTest_getTransactions, invalidAddress) {
  ...
  ASSERT_EQ(make_error_code(VarNote::error::BAD_ADDRESS), ec);
}
...
TEST_F(WalletServiceTest_getTransactions, invalidPaymentId) {
  ...
  ASSERT_EQ(make_error_code(VarNote::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT), ec);
}
...
TEST_F(WalletServiceTest_getTransactions, blockNotFound) {
  ...
  auto ec = service->getTransactions({}, 0, 1, "", transactions);
  ASSERT_EQ(make_error_code(VarNote::error::WalletServiceErrorCode::OBJECT_NOT_FOUND), ec);
}
...
TEST_F(WalletServiceTest_getTransaction, wrongHash) {
  ...
  ASSERT_EQ(make_error_code(VarNote::error::WalletServiceErrorCode::WRONG_HASH_FORMAT), ec);
}
...
public:
  virtual void SetUp() override;
protected:
  std::vector<VarNote::WalletTransactionWithTransfers> transactions;
};

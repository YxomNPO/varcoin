...
TEST(getAccountAddressAsStr, works_correctly)
{
  ...
  ASSERT_NO_THROW(VarNote::loadFromBinary(addr, Common::asBinaryArray(test_serialized_keys)));
  std::string addr_str = VarNote::getAccountAddressAsStr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, addr);
...
TEST(parseAccountAddressString, handles_valid_address)
{
  uint64_t prefix;
  VarNote::AccountPublicAddress addr;
  ASSERT_TRUE(VarNote::parseAccountAddressString(prefix, addr, test_keys_addr_str));
  ASSERT_EQ(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, prefix);

  BinaryArray blob;
  ASSERT_NO_THROW(blob = VarNote::storeToBinary(addr));
...
TEST(parseAccountAddressString, fails_on_invalid_address_format)
{
  ...
  uint64_t prefix;
  VarNote::AccountPublicAddress addr;
  ASSERT_FALSE(VarNote::parseAccountAddressString(prefix, addr, addr_str));
}

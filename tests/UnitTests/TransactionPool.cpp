...
#include "VarNoteCore/ITransactionValidator.h"
#include "VarNoteCore/ITimeProvider.h"
#include "VarNoteCore/TransactionExtra.h"
#include "VarNoteCore/TransactionPool.h"
...
class TransactionValidator : public VarNote::ITransactionValidator {
  virtual bool checkTransactionInputs(const VarNote::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const VarNote::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const VarNote::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const VarNote::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const VarNote::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const VarNote::Currency& currency, Logging::ILogger& logger) {}
  };
  ...
  public:
    ...
    VarNote::RealTimeProvider m_time;
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    // TODO fix it
    //currency = VarNote::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }

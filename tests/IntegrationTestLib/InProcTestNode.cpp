...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const VarNote::Currency& currency) : 
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new VarNote::core(m_currency, NULL, log));
    protocol.reset(new VarNote::VarNoteProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new VarNote::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_varnote_protocol(protocol.get());

    VarNote::NetNodeConfig p2pConfig;
    ...
    VarNote::MinerConfig emptyMiner;
  ...
  core->deinit();
  p2pNode->deinit();
  core->set_varnote_protocol(NULL);
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, VarNote::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<VarNote::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new VarNote::InProcessNode(*core, *protocol));

...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const VarNote::Currency& currency, System::Dispatcher& d) : 
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    core.reset(new VarNote::Core(
      m_currency,
      log,
      VarNote::Checkpoints(log),
      dispatcher,
      std::unique_ptr<IBlockchainCacheFactory>(new MemoryBlockchainCacheFactory("", logger.getLogger())),
      VarNote::createVectorMainChainStorage(m_currency)));
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, VarNote::BlockTemplate &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<VarNote::INode> &node) {
  ...
  std::unique_ptr<InProcessNode> inprocNode(new VarNote::InProcessNode(*core, *protocol, dispatcher));

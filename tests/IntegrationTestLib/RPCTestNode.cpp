...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, VarNote::BlockTemplate& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<VarNote::INode>& node) {
  std::unique_ptr<VarNote::INode> newNode(new VarNote::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    VarNote::COMMAND_RPC_GET_INFO::request req;
    VarNote::COMMAND_RPC_GET_INFO::response rsp;

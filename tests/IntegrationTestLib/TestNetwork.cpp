...
#ifdef _WIN32
const std::string varcoinDaemon = "varcoind.exe";
#else
const std::string varcoinDaemon = "varcoind";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.daemonPath = varcoinDaemon; // default
  cfg.testnet = testnet;
  cfg.logFile = "test_varcoind" + std::to_string(index) + ".log";
...
TestNetwork::TestNetwork(System::Dispatcher& dispatcher, const VarNote::Currency& currency) : 

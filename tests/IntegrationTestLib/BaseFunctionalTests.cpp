...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "varcoind.exe";
#else
const std::string DAEMON_FILENAME = "varcoind";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_varcoind_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"varcoind" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "varcoind", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public VarNote::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    VarNote::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, VarNote::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(VarNote::TransactionId transactionId) override {
      VarNote::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const VarNote::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, VarNote::BlockTemplate&& blockTemplate) {
  ...
  if (blockTemplate.majorVersion >= BLOCK_MAJOR_VERSION_2) {
    ...
    mmTag.depth = 0;
    //FIXME
    /* if (!VarNote::get_aux_block_header_hash(blockTemplate, mmTag.merkleRoot)) { */
    /*   return false; */
    /* } */

    blockTemplate.parentBlock.baseTransaction.extra.clear();
    if (!VarNote::appendMergeMiningTagToExtra(blockTemplate.parentBlock.baseTransaction.extra, mmTag)) {
      return false;
    }
  }

  BinaryArray blockBlob = VarNote::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<VarNote::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<VarNote::IWalletLegacy> & wallet, std::unique_ptr<VarNote::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<VarNote::IWalletLegacy>(new VarNote::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : VarNote::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(VarNote::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, VarNote::INode& node, size_t expectedPoolSize,
  std::vector<std::unique_ptr<VarNote::ITransactionReader>>& txPool) {
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, VarNote::INode& node,
  std::vector<std::unique_ptr<VarNote::ITransactionReader>>& txPool) {

  ...
  namespace po = boost::program_options;
    ...
    public:
      ...
      void init(po::options_description& desc) {
        desc.add_options()
          ("daemon-dir,d", po::value<std::string>()->default_value("."), "path to varcoind.exe")
    ...
    public:
      BaseFunctionalTests(const VarNote::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<VarNote::IWalletLegacy> & wallet, std::unique_ptr<VarNote::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const VarNote::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<VarNote::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, VarNote::INode& node, std::vector<std::unique_ptr<VarNote::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(VarNote::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, VarNote::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<VarNote::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, VarNote::BlockTemplate&& blockTemplate);
      ...
      std::unique_ptr<VarNote::INode> mainNode;
      std::unique_ptr<VarNote::IWalletLegacy> workingWallet;

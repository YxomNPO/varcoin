...
public:
  ...
  bool init()
  {
    ...
    for (uint32_t i = 0; i < ring_size; ++i)
    {
      ...
      if (!currency.constructMinerTx(VarNote::BLOCK_MAJOR_VERSION_1, 0, 0, 0, 2, 0, m_miners[i].getAccountKeys().address, m_miner_txs[i]))
        return false;
...
protected:
  VarNote::AccountBase m_miners[ring_size];
  VarNote::Transaction m_miner_txs[ring_size];

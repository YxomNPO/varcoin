...
class TrivialWalletObserver : public VarNote::IWalletObserver
{
public:
  ...
  virtual void sendTransactionCompleted(VarNote::TransactionId transactionId, std::error_code result) {
...
VarNote::TransactionId TransferMoney(VarNote::Wallet& from, VarNote::Wallet& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  VarNote::Transfer transfer;
...
protected:
  ...
  void GetOneBlockReward(VarNote::Wallet& wallet);
...
void WalletApi::prepareAliceWallet() {
  ...
  alice.reset(new VarNote::Wallet(*aliceNode));
...
void WalletApi::prepareBobWallet() {
  ...
  bob.reset(new VarNote::Wallet(*bobNode));
...
void WalletApi::prepareCarolWallet() {
  ...
  carol.reset(new VarNote::Wallet(*carolNode));
  carol->addObserver(carolWalletObserver.get());
}

void WalletApi::GetOneBlockReward(VarNote::Wallet& wallet) {
  VarNote::account_public_address address;
  ASSERT_TRUE(VarNote::get_account_address_from_str(address, wallet.getAddress()));
...
void WalletApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<VarNote::Transfer> trs;
  VarNote::Transfer tr;
...
TEST_F(WalletApi, refreshWithMoney) {
  ...
  VarNote::account_public_address address;
  ASSERT_TRUE(VarNote::get_account_address_from_str(address, alice->getAddress()));
  generator.getBlockRewardForAddress(address);
...
TEST_F(WalletApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, VarNote::INVALID_TRANSFER_ID);

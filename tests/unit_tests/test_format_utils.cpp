...
TEST(parse_tx_extra, handles_empty_extra)
{
  ...
  std::vector<VarNote::tx_extra_field> tx_extra_fields;
  ASSERT_TRUE(VarNote::parse_tx_extra(extra, tx_extra_fields));
...
TEST(parse_tx_extra, handles_padding_only_size_1)
{
  ...
  ASSERT_EQ(typeid(VarNote::tx_extra_padding), tx_extra_fields[0].type());
  ASSERT_EQ(1, boost::get<VarNote::tx_extra_padding>(tx_extra_fields[0]).size);
}
...
TEST(parse_tx_extra, handles_padding_only_size_2)
{
  ...
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(VarNote::tx_extra_padding), tx_extra_fields[0].type());
  ASSERT_EQ(2, boost::get<VarNote::tx_extra_padding>(tx_extra_fields[0]).size);
}
...
TEST(parse_tx_extra, handles_padding_only_max_size)
{
  ...
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(VarNote::tx_extra_padding), tx_extra_fields[0].type());
  ASSERT_EQ(TX_EXTRA_NONCE_MAX_COUNT, boost::get<VarNote::tx_extra_padding>(tx_extra_fields[0]).size);
}
...
TEST(parse_tx_extra, handles_padding_only_exceed_max_size)
{
  ...
  std::vector<VarNote::tx_extra_field> tx_extra_fields;
  ASSERT_FALSE(VarNote::parse_tx_extra(extra, tx_extra_fields));
}
...
TEST(parse_tx_extra, handles_pub_key_only)
{
  ...
  ASSERT_TRUE(VarNote::parse_tx_extra(extra, tx_extra_fields));
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(VarNote::tx_extra_pub_key), tx_extra_fields[0].type());
}
...
TEST(parse_tx_extra, handles_extra_nonce_only)
{
  ...
  ASSERT_TRUE(VarNote::parse_tx_extra(extra, tx_extra_fields));
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(VarNote::tx_extra_nonce), tx_extra_fields[0].type());
  VarNote::tx_extra_nonce extra_nonce = boost::get<VarNote::tx_extra_nonce>(tx_extra_fields[0]);
...
TEST(parse_tx_extra, handles_pub_key_and_padding)
{
  ...
  ASSERT_EQ(typeid(VarNote::tx_extra_pub_key), tx_extra_fields[0].type());
  ASSERT_EQ(typeid(VarNote::tx_extra_padding), tx_extra_fields[1].type());
}

TEST(parse_and_validate_tx_extra, is_valid_tx_extra_parsed)
{
  VarNote::transaction tx = AUTO_VAL_INIT(tx);
  VarNote::account_base acc;
  acc.generate();
  VarNote::blobdata b = "dsdsdfsdfsf";
  ASSERT_TRUE(VarNote::construct_miner_tx(0, 0, 10000000000000, 1000, DEFAULT_FEE, acc.get_keys().m_account_address, tx, b, 1));
  crypto::public_key tx_pub_key = VarNote::get_tx_pub_key_from_extra(tx);
  ASSERT_NE(tx_pub_key, VarNote::null_pkey);
}
TEST(parse_and_validate_tx_extra, fails_on_big_extra_nonce)
{
  ...
  VarNote::account_base acc;
  acc.generate();
  VarNote::blobdata b(TX_EXTRA_NONCE_MAX_COUNT + 1, 0);
  ASSERT_FALSE(VarNote::construct_miner_tx(0, 0, 10000000000000, 1000, DEFAULT_FEE, acc.get_keys().m_account_address, tx, b, 1));
}
TEST(parse_and_validate_tx_extra, fails_on_wrong_size_in_extra_nonce)
{
  ...
  tx.extra[1] = 255;
  std::vector<VarNote::tx_extra_field> tx_extra_fields;
  ASSERT_FALSE(VarNote::parse_tx_extra(tx.extra, tx_extra_fields));
}
TEST(validate_parse_amount_case, validate_parse_amount)
{
  uint64_t res = 0;
  bool r = VarNote::parse_amount(res, "0.0001");
  ASSERT_TRUE(r);
  ASSERT_EQ(res, 10000);

  r = VarNote::parse_amount(res, "100.0001");
  ASSERT_TRUE(r);
  ASSERT_EQ(res, 10000010000);

  r = VarNote::parse_amount(res, "000.0000");
  ASSERT_TRUE(r);
  ASSERT_EQ(res, 0);

  r = VarNote::parse_amount(res, "0");
  ASSERT_TRUE(r);
  ASSERT_EQ(res, 0);
  ...
  r = VarNote::parse_amount(res, "   100.0000    ");
  ASSERT_TRUE(r);
  ...
  r = VarNote::parse_amount(res, "100.00 00");
  ASSERT_FALSE(r);

  r = VarNote::parse_amount(res, "1 00.00 00");
  ASSERT_FALSE(r);
}

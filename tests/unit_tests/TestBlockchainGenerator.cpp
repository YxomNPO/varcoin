...
public:
  ...
  void generate(const VarNote::account_public_address& address, VarNote::transaction& tx)
  {
    VarNote::tx_destination_entry destination(this->m_source_amount, address);
    std::vector<VarNote::tx_destination_entry> destinations;
    destinations.push_back(destination);

    VarNote::construct_tx(this->m_miners[this->real_source_idx].get_keys(), this->m_sources, destinations, std::vector<uint8_t>(), tx, 0);
  }
};
...
std::vector<VarNote::block>& TestBlockchainGenerator::getBlockchain()
{
  return m_blockchain;
}

bool TestBlockchainGenerator::getTransactionByHash(const crypto::hash& hash, VarNote::transaction& tx)
{
...
void TestBlockchainGenerator::generateEmptyBlocks(size_t count)
{
  ...
  {
    VarNote::block& prev_block = m_blockchain.back();
    VarNote::block block;
...
{
  crypto::hash txHash = VarNote::get_transaction_hash(transaction);
...
bool TestBlockchainGenerator::getBlockRewardForAddress(const VarNote::account_public_address& address)
{
  ...
  crypto::hash txHash = VarNote::get_transaction_hash(tx);
  m_txs[txHash] = tx;

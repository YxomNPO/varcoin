...
void INodeTrivialRefreshStub::getNewBlocks(std::list<crypto::hash>&& knownBlockIds, std::list<VarNote::block_complete_entry>& newBlocks, uint64_t& startHeight, const Callback& callback)
{
  std::thread task(&INodeTrivialRefreshStub::doGetNewBlocks, this, knownBlockIds, std::ref(newBlocks), std::ref(startHeight), callback);
  task.detach();
}

void INodeTrivialRefreshStub::doGetNewBlocks(std::list<crypto::hash> knownBlockIds, std::list<VarNote::block_complete_entry>& newBlocks, uint64_t& startHeight, const Callback& callback)
{
  ...
  {
    VarNote::block_complete_entry e;
    e.block = VarNote::t_serializable_object_to_blob(blockchain[m_lastHeight]);
...
void INodeTrivialRefreshStub::relayTransaction(const VarNote::transaction& transaction, const Callback& callback)
{
  std::thread task(&INodeTrivialRefreshStub::doRelayTransaction, this, transaction, callback);
  task.detach();
}

void INodeTrivialRefreshStub::doRelayTransaction(const VarNote::transaction& transaction, const Callback& callback)
{
  if (m_nextTxError)
  {
    callback(make_error_code(VarNote::error::INTERNAL_WALLET_ERROR));
    m_nextTxError = false;
    return;
  }
...
void INodeTrivialRefreshStub::getRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount, std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result, const Callback& callback)
{
  std::thread task(&INodeTrivialRefreshStub::doGetRandomOutsByAmounts, this, amounts, outsCount, std::ref(result), callback);
  task.detach();
}

void INodeTrivialRefreshStub::doGetRandomOutsByAmounts(std::vector<uint64_t> amounts, uint64_t outsCount, std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result, const Callback& callback)
{
  for (uint64_t amount: amounts)
  {
    VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount out;
    out.amount = amount;
    ...
    for (uint64_t i = 0; i < outsCount; ++i)
    {
      ...
      VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry e;
...
{
  std::vector<VarNote::block>& blockchain = m_blockchainGenerator.getBlockchain();

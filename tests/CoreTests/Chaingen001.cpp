...
bool one_block::verify_1(VarNote::Core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
    ...
    //map_hash2tx_t mtx;
    //CHECK_TEST_CONDITION(find_block_chain(events, chain, mtx, getBlockHash(boost::get<VarNote::BlockTemplate>(events[1]))));
    ...
    //CHECK_TEST_CONDITION(blocks.back() == boost::get<VarNote::BlockTemplate>(events[0]));
...
bool gen_simple_chain_001::generate(std::vector<test_event_entry> &events)
{
    ...
    map_hash2tx_t mtx;
    /*bool r = */find_block_chain(events, chain, mtx, getBlockHash(boost::get<VarNote::BlockTemplate>(events[3])));

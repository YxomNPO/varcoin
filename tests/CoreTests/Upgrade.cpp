...
namespace {
  bool makeBlocks(std::vector<test_event_entry>& events, test_generator& generator, BlockTemplate& lastBlock,
                  const BlockTemplate& parentBlock, const VarNote::AccountBase& minerAcc, size_t count,
                  uint8_t majorVersion, uint8_t minorVersion) {
   VarNote::BlockTemplate prevBlock = parentBlock;
    for (size_t i = 0; i < count; ++i) {
     VarNote::BlockTemplate b;
...
gen_upgrade::gen_upgrade() : m_invalidBlockIndex(0), m_checkBlockTemplateVersionCallCounter(0),
  ...
  currencyBuilder.upgradeHeightV3(VarNote::parameters::VARNOTE_MAX_BLOCK_NUMBER);
...
bool gen_upgrade::checkBeforeUpgrade(std::vector<test_event_entry>& events, test_generator& generator,
                                     const VarNote::BlockTemplate& parentBlock, const VarNote::AccountBase& minerAcc,
...
bool gen_upgrade::check_block_verification_context(std::error_code bve, size_t eventIdx, const VarNote::BlockTemplate& /*blk*/) {
...
bool gen_upgrade::markInvalidBlock(VarNote::Core& /*c*/, size_t evIndex, const std::vector<test_event_entry>& /*events*/) {
  m_invalidBlockIndex = evIndex + 1;
  return true;
}

bool gen_upgrade::checkBlockTemplateVersionIsV1(VarNote::Core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
...
bool gen_upgrade::checkBlockTemplateVersionIsV2(VarNote::Core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
...
bool gen_upgrade::checkBlockTemplateVersion(VarNote::Core& c, uint8_t expectedMajorVersion, uint8_t expectedMinorVersion) {
...
bool gen_upgrade::checkBlockRewardEqFee(VarNote::Core& c, size_t evIndex, const std::vector<test_event_entry>& events) {
...
bool gen_upgrade::checkBlockRewardIsZero(VarNote::Core& c, size_t evIndex, const std::vector<test_event_entry>& events) {
...
bool gen_upgrade::rememberCoinsInCirculationBeforeUpgrade(VarNote::Core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
  m_coinsInCirculationBeforeUpgrade = c.getTotalGeneratedAmount();
  return true;
}

bool gen_upgrade::rememberCoinsInCirculationAfterUpgrade(VarNote::Core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {

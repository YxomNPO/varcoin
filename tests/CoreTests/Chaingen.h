...
struct serialized_object
{
  ...
  serialized_object(const VarNote::BinaryArray& a_data)
    : data(a_data)
  {
  }
...
typedef boost::variant<VarNote::Block, VarNote::Transaction, VarNote::AccountBase, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
...
public:
  test_chain_unit_base() :
    m_currency(VarNote::CurrencyBuilder(m_logger).currency()) {
  }

  typedef std::function<bool (VarNote::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, VarNote::Transaction& tx,
                         const VarNote::Block& blk_head, const VarNote::AccountBase& from, const VarNote::AccountBase& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
VarNote::Transaction construct_tx_with_fee(Logging::ILogger& logger, std::vector<test_event_entry>& events, const VarNote::Block& blk_head,
                                            const VarNote::AccountBase& acc_from, const VarNote::AccountBase& acc_to,
...
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const VarNote::Block& blk_head,
                                      const VarNote::AccountBase& from, const VarNote::AccountBase& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<VarNote::TransactionSourceEntry>& sources,
                                      std::vector<VarNote::TransactionDestinationEntry>& destinations);
uint64_t get_balance(const VarNote::AccountBase& addr, const std::vector<VarNote::Block>& blockchain, const map_hash2tx_t& mtx);
...
public:
  ...
  bool operator()(const VarNote::Block& b) const
  {
    ...
    VarNote::block_verification_context bvc = boost::value_initialized<decltype(bvc)>();
...
inline bool do_replay_events(std::vector<test_event_entry>& events, t_test_class& validator)
{
  ...
  VarNote::CoreConfig::initOptions(desc);

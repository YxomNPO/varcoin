...
bool GetRandomOutputs::request(VarNote::Core& c, uint64_t ramount, size_t mixin,
  ...
  for (auto amount : req.amounts) {
    ...
    resp.outs.emplace_back(VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount{amount, {}});
...
bool GetRandomOutputs::checkHalfUnlocked(VarNote::Core& c, size_t ev_index,
...
bool GetRandomOutputs::checkFullyUnlocked(VarNote::Core& c, size_t ev_index,
                                          const std::vector<test_event_entry>& events) {
  VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;

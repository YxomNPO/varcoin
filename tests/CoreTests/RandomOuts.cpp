...
bool GetRandomOutputs::request(VarNote::core& c, uint64_t amount, size_t mixin, VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp) {
  VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request req;

  req.amounts.push_back(amount);
  req.outs_count = mixin;

  resp = boost::value_initialized<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response>();
...
bool GetRandomOutputs::checkHalfUnlocked(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
...
bool GetRandomOutputs::checkFullyUnlocked(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;

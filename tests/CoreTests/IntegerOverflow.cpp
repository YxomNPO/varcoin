...
namespace
{
  ...
  void append_TransactionSourceEntry(std::vector<VarNote::TransactionSourceEntry>& sources, const Transaction& tx, size_t out_idx)
  {
    VarNote::TransactionSourceEntry se;
    se.amount = tx.outputs[out_idx].amount;
    se.outputs.push_back(std::make_pair(0, boost::get<VarNote::KeyOutput>(tx.outputs[out_idx].target).key));
    se.realOutput = 0;

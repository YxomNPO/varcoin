...
//-----------------------------------------------------------------------------------------------------
bool gen_chain_switch_1::check_split_not_switched(VarNote::Core& c, size_t ev_index,
  ...
  auto blk_4 = VarNote::fromBinaryArray<BlockTemplate>(
...
//-----------------------------------------------------------------------------------------------------
bool gen_chain_switch_1::check_split_switched(VarNote::Core& c, size_t ev_index,
                                              const std::vector<test_event_entry>& events) {
  ...
  for (const auto& b : alt_blocks) {
    CHECK_TEST_CONDITION(
        !(m_chain_1.end() == std::find_if(m_chain_1.cbegin(), m_chain_1.cend(),
                                          [&b](const VarNote::BlockTemplate& arg) -> bool { return b == arg; })));
  }

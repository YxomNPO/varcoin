...
#include "VarNoteCore/UpgradeDetector.h"

//#include "AccountBoostSerialization.h"
//#include "varnote_boost_serialization.h"

using namespace std;
using namespace VarNote;

struct output_index {
    const VarNote::TransactionOutputTarget out;
    uint64_t amount;
    ...
    output_index(const VarNote::TransactionOutputTarget &_out, uint64_t _a, size_t _h, size_t tno, size_t ono, const VarNote::Block *_pb, const VarNote::Transaction *_pt)
...
bool init_output_indices(map_output_idx_t& outs, std::map<uint64_t, std::vector<size_t> >& outs_mine, const std::vector<VarNote::Block>& blockchain, const map_hash2tx_t& mtx, const VarNote::AccountBase& from) {
...
bool init_spent_output_indices(map_output_idx_t& outs, map_output_t& outs_mine, const std::vector<VarNote::Block>& blockchain, const map_hash2tx_t& mtx, const VarNote::AccountBase& from) {
...
bool fill_tx_sources(std::vector<TransactionSourceEntry>& sources, const std::vector<test_event_entry>& events,
                     const Block& blk_head, const VarNote::AccountBase& from, uint64_t amount, size_t nmix)
{
    ...
    BOOST_REVERSE_FOREACH(const map_output_t::value_type o, outs_mine)
    {
        for (size_t i = 0; i < o.second.size() && !sources_found; ++i)
        {
            ...
            VarNote::TransactionSourceEntry ts;
            ts.amount = oi.amount;
...
bool fill_tx_destination(TransactionDestinationEntry &de, const VarNote::AccountBase &to, uint64_t amount) {
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, VarNote::Transaction& tx, const Block& blk_head,
...
const VarNote::Currency& test_chain_unit_base::currency() const
{
  return m_currency;
}

...
struct output_index {
  ...
  uint32_t idx;
  bool spent;
  VarNote::BlockTemplate p_blk;
  VarNote::Transaction p_tx;

  output_index(const VarNote::TransactionOutputTarget& _out, uint64_t _a, size_t _h, size_t tno, size_t ono,
               const VarNote::BlockTemplate& _pb, const VarNote::Transaction& _pt)
...
bool init_output_indices(map_output_idx_t& outs, std::map<uint64_t, std::vector<size_t>>& outs_mine,
                         const std::vector<VarNote::BlockTemplate>& blockchain, const map_hash2tx_t& mtx,
...
bool fill_tx_sources(std::vector<TransactionSourceEntry>& sources, const std::vector<test_event_entry>& events,
                     const BlockTemplate& blk_head, const VarNote::AccountBase& from, uint64_t amount, size_t nmix) {
  map_output_idx_t outs;
  map_output_t outs_mine;

  std::vector<VarNote::BlockTemplate> blockchain;
  map_hash2tx_t mtx;
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events,
                         VarNote::Transaction& tx, const BlockTemplate& blk_head,
...
bool test_chain_unit_base::verify(const std::string& cb_name, VarNote::Core& c, size_t ev_index,

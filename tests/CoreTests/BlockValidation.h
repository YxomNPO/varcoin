...
public:
  CheckBlockPurged(size_t invalidBlockIdx, uint8_t blockMajorVersion) :
    ...
    assert(blockMajorVersion == VarNote::BLOCK_MAJOR_VERSION_1 || blockMajorVersion == VarNote::BLOCK_MAJOR_VERSION_2);

    VarNote::CurrencyBuilder currencyBuilder(m_logger);
    currencyBuilder.upgradeHeightV2(blockMajorVersion == VarNote::BLOCK_MAJOR_VERSION_1 ? VarNote::IUpgradeDetector::UNDEF_HEIGHT : UINT32_C(0));
  ...
  bool check_block_verification_context(std::error_code bve, size_t eventIdx, const VarNote::RawBlock& /*blk*/) {
    return check_block_verification_context(bve, eventIdx, VarNote::BlockTemplate{});
  }

  bool check_block_purged(VarNote::Core& c, size_t eventIdx, const std::vector<test_event_entry>& events) {
  ...
  bool markInvalidBlock(VarNote::Core& c, size_t eventIdx, const std::vector<test_event_entry>& events) {
...
struct CheckBlockAccepted : public test_chain_unit_base {
  ...
  bool check_block_accepted(VarNote::Core& c, size_t /*eventIdx*/, const std::vector<test_event_entry>& /*events*/) {
...
struct gen_block_ts_not_checked : public CheckBlockAccepted
{
  gen_block_ts_not_checked(uint8_t blockMajorVersion)
    ...
    // bad bad bad
    const_cast<VarNote::BlockTemplate&>(m_currency->genesisBlock()).timestamp = 1338224400;
...
struct gen_block_invalid_binary_format : public test_chain_unit_base
{
  ...
  bool generate(std::vector<test_event_entry>& events) const;
  bool check_block_verification_context(std::error_code bve, size_t event_idx, const VarNote::BlockTemplate& /*blk*/);
  bool check_all_blocks_purged(VarNote::Core& c, size_t ev_index, const std::vector<test_event_entry>& events);
  bool corrupt_blocks_boundary(VarNote::Core& c, size_t ev_index, const std::vector<test_event_entry>& events);
...
struct TestMaxSizeOfParentBlock : public CheckBlockAccepted {
  TestMaxSizeOfParentBlock() : CheckBlockAccepted(2, VarNote::BLOCK_MAJOR_VERSION_2) {
  }
...
struct TestBigParentBlock : public CheckBlockPurged {
  TestBigParentBlock() : CheckBlockPurged(1, VarNote::BLOCK_MAJOR_VERSION_2) {
  }
...
struct TestBlock2ExtraEmpty : public CheckBlockPurged {
  ...
  TestBlock2ExtraEmpty() : CheckBlockPurged(1, VarNote::BLOCK_MAJOR_VERSION_2) {}
...
struct TestBlock2ExtraWithoutMMTag : public CheckBlockPurged {
  ...
  TestBlock2ExtraWithoutMMTag() : CheckBlockPurged(1, VarNote::BLOCK_MAJOR_VERSION_2) {}
...
struct TestBlock2ExtraWithGarbage : public CheckBlockAccepted {
  ...
  TestBlock2ExtraWithGarbage() : CheckBlockAccepted(2, VarNote::BLOCK_MAJOR_VERSION_2) {}

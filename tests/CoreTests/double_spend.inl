...
bool gen_double_spend_base<concrete_test>::check_double_spend(VarNote::core& c, size_t /*ev_index*/, const std::vector<test_event_entry>& events)
{
  ...
  VarNote::AccountBase bob_account = boost::get<VarNote::AccountBase>(events[1]);
  VarNote::AccountBase alice_account = boost::get<VarNote::AccountBase>(events[2]);
...
bool gen_double_spend_in_tx<txs_keeped_by_block>::generate(std::vector<test_event_entry>& events) const
{
  ...
  VarNote::TransactionSourceEntry se;
  se.amount = tx_0.outputs[0].amount;
  se.outputs.push_back(std::make_pair(0, boost::get<VarNote::KeyOutput>(tx_0.outputs[0].target).key));
  se.realOutput = 0;
  se.realTransactionPublicKey = VarNote::getTransactionPublicKeyFromExtra(tx_0.extra);

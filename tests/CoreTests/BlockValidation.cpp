...
bool lift_up_difficulty(const VarNote::Currency& currency, std::vector<test_event_entry>& events,
                        ...
                        std::vector<VarNote::Difficulty>& cummulative_difficulties, test_generator& generator,
                        size_t new_block_count, const VarNote::BlockTemplate blk_last,
                        const VarNote::AccountBase& miner_account, uint8_t block_major_version) {
  VarNote::Difficulty commulative_diffic = cummulative_difficulties.empty() ? 0 : cummulative_difficulties.back();
  VarNote::BlockTemplate blk_prev = blk_last;
  for (size_t i = 0; i < new_block_count; ++i) {
    VarNote::BlockTemplate blk_next;
    VarNote::Difficulty diffic = currency.nextDifficulty(timestamps, cummulative_difficulties);
...
bool getParentBlockSize(const VarNote::BlockTemplate& block, size_t& size) {
...
bool adjustParentBlockSize(VarNote::BlockTemplate& block, size_t targetSize) {
...
bool gen_block_invalid_prev_id::check_block_verification_context(std::error_code bve, size_t event_idx,
                                                                 const VarNote::BlockTemplate& /*blk*/) {
  if (1 == event_idx)
    return bve == VarNote::error::AddBlockErrorCode::REJECTED_AS_ORPHANED;
  else
    return bve == VarNote::error::AddBlockErrorCode::ADDED_TO_MAIN;
}
...
gen_block_invalid_binary_format::gen_block_invalid_binary_format(uint8_t blockMajorVersion)
  ...
  currencyBuilder.upgradeHeightV2(blockMajorVersion == VarNote::BLOCK_MAJOR_VERSION_1 ? IUpgradeDetector::UNDEF_HEIGHT : UINT32_C(0));
...
bool gen_block_invalid_binary_format::check_block_verification_context(std::error_code bve, size_t event_idx,
                                                                       const VarNote::BlockTemplate& blk) {
  using VarNote::error::AddBlockErrorCode;
...
bool gen_block_invalid_binary_format::corrupt_blocks_boundary(VarNote::Core& c, size_t ev_index,
                                                              const std::vector<test_event_entry>& events) {
  m_corrupt_blocks_begin_idx = ev_index + 1;
  return true;
}

bool gen_block_invalid_binary_format::check_all_blocks_purged(VarNote::Core& c, size_t ev_index,
                                                              const std::vector<test_event_entry>& events) {
...
bool GenerateAndMutateBlockV2(const VarNote::Currency& currency, std::vector<test_event_entry>& events,
...
bool TestBlock2ExtraEmpty::generate(std::vector<test_event_entry>& events) const {
  return GenerateAndMutateBlockV2(*m_currency, events, "check_block_purged", [](VarNote::BlockTemplate& blk) {
...
bool TestBlock2ExtraWithGarbage::generate(std::vector<test_event_entry>& events) const {
  return GenerateAndMutateBlockV2(*m_currency, events, "check_block_accepted", [](VarNote::BlockTemplate& blk) {
    VarNote::addExtraNonceToTransactionExtra(blk.parentBlock.baseTransaction.extra, asBinaryArray("0xdeadbeef"));

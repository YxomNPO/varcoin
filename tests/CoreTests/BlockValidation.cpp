...
namespace {
  bool lift_up_difficulty(const VarNote::Currency& currency, std::vector<test_event_entry>& events,
                          std::vector<uint64_t>& timestamps,
                          std::vector<VarNote::difficulty_type>& cummulative_difficulties, test_generator& generator,
                          size_t new_block_count, const VarNote::Block blk_last,
                          const VarNote::AccountBase& miner_account, uint8_t block_major_version) {
    VarNote::difficulty_type commulative_diffic = cummulative_difficulties.empty() ? 0 : cummulative_difficulties.back();
    VarNote::Block blk_prev = blk_last;
    for (size_t i = 0; i < new_block_count; ++i) {
      VarNote::Block blk_next;
      VarNote::difficulty_type diffic = currency.nextDifficulty(timestamps, cummulative_difficulties);
  ...
  bool getParentBlockSize(const VarNote::Block& block, size_t& size) {
    auto serializer = VarNote::makeParentBlockSerializer(block, false, false);
    if (!VarNote::getObjectBinarySize(serializer, size)) {
  ...
  bool adjustParentBlockSize(VarNote::Block& block, size_t targetSize) {
  ...
  void clearTransaction(VarNote::Transaction& tx) {
    tx.version = 0;
    tx.unlockTime = 0;
    tx.inputs.clear();
    tx.outputs.clear();
...
bool gen_block_invalid_prev_id::check_block_verification_context(const VarNote::block_verification_context& bvc, size_t event_idx, const VarNote::Block& /*blk*/)
{
  if (1 == event_idx)
...
gen_block_invalid_binary_format::gen_block_invalid_binary_format(uint8_t blockMajorVersion) : 
  ...
  VarNote::CurrencyBuilder currencyBuilder(m_logger);
  currencyBuilder.upgradeHeight(blockMajorVersion == VarNote::BLOCK_MAJOR_VERSION_1 ? UNDEF_HEIGHT : 0);
...
bool gen_block_invalid_binary_format::check_block_verification_context(const VarNote::block_verification_context& bvc,
                                                                       size_t event_idx, const VarNote::Block& blk)
{
...
bool gen_block_invalid_binary_format::corrupt_blocks_boundary(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  m_corrupt_blocks_begin_idx = ev_index + 1;
  return true;
}

bool gen_block_invalid_binary_format::check_all_blocks_purged(VarNote::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
...
namespace
{
  ...
  bool GenerateAndMutateBlockV2(const VarNote::Currency& currency, std::vector<test_event_entry>& events, const std::string& callback, MutateFunc mf) {
...
bool TestBlock2ExtraEmpty::generate(std::vector<test_event_entry>& events) const {
  return GenerateAndMutateBlockV2(m_currency, events, "check_block_purged", [](VarNote::Block& blk) {
...
bool TestBlock2ExtraWithoutMMTag::generate(std::vector<test_event_entry>& events) const {
  return GenerateAndMutateBlockV2(m_currency, events, "check_block_purged", [](VarNote::Block& blk) {
    blk.parentBlock.baseTransaction.extra.clear();
    VarNote::addExtraNonceToTransactionExtra(blk.parentBlock.baseTransaction.extra, asBinaryArray("0xdeadbeef"));
  });
}

bool TestBlock2ExtraWithGarbage::generate(std::vector<test_event_entry>& events) const {
  return GenerateAndMutateBlockV2(m_currency, events, "check_block_accepted", [](VarNote::Block& blk) {
    VarNote::addExtraNonceToTransactionExtra(blk.parentBlock.baseTransaction.extra, asBinaryArray("0xdeadbeef"));

...
TransactionBuilder::TransactionBuilder(const VarNote::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(VarNote::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const VarNote::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<VarNote::TransactionSourceEntry>& sources, const VarNote::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<VarNote::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const VarNote::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<VarNote::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<VarNote::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<VarNote::KeyPair>& contexts, Transaction& tx) const {

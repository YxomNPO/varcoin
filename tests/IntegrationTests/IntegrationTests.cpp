...
public:
  ...
  IntegrationTest() : 
    currency(VarNote::CurrencyBuilder(log).testnet(true).currency()), 
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<VarNote::IWalletLegacy> wallet(new VarNote::WalletLegacy(m_currency, *n));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<VarNote::INode>& localNode = inodes.front();
    std::unique_ptr<VarNote::INode>& remoteNode = inodes.back();

...
public:
  ...
  SimpleTest(const VarNote::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) : 
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public VarNote::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public VarNote::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public VarNote::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public VarNote::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public VarNote::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    VarNote::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, VarNote::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public VarNote::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<VarNote::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public VarNote::IWalletLegacyObserver {
  public:
    ...
    VarNote::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<VarNote::INode> node1;
    std::unique_ptr<VarNote::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<VarNote::IWalletLegacy> wallet1;
    std::unique_ptr<VarNote::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public VarNote::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public VarNote::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<VarNote::INode> localNode;
    std::unique_ptr<VarNote::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<VarNote::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<VarNote::INode> hopNode;
    std::unique_ptr<VarNote::INode> localNode;
    std::unique_ptr<VarNote::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<VarNote::INode> node1;
    std::unique_ptr<VarNote::INode> inprocNode;
...
void testMultiVersion(const VarNote::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = VarNote::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();

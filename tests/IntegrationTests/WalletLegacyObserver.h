...
public:
  ...
  VarNote::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    VarNote::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(VarNote::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<VarNote::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<VarNote::TransactionId, std::error_code> m_sendResults;
  std::deque<VarNote::TransactionId> m_externalTransactions;
};

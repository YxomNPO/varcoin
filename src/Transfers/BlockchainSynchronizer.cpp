...
std::ostream& operator<<(std::ostream& os, const VarNote::IBlockchainConsumer* consumer) {
...
public:
  explicit TransactionReaderListFormatter(const std::vector<std::unique_ptr<VarNote::ITransactionReader>>& transactionList) :
...
private:
  const std::vector<std::unique_ptr<VarNote::ITransactionReader>>& m_transactionList;
};

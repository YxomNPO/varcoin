...
TransfersSyncronizer::TransfersSyncronizer(const VarNote::Currency& currency, Logging::ILogger& logger, IBlockchainSynchronizer& sync, INode& node) :
...
void TransfersSyncronizer::save(std::ostream& os) {
  ...
  VarNote::BinaryOutputStreamSerializer s(stream);
...
void TransfersSyncronizer::load(std::istream& is) {
  ...
  VarNote::BinaryInputStreamSerializer s(inputStream);
  uint32_t version = 0;

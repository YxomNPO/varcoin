...
std::string get_default_data_directory(
    const std::string &varnote_name);  // we avoid including app-specific headers into our platform code
// Old method
...
// Mac: ~/.VARNOTE_NAME
// Unix: ~/.VARNOTE_NAME
// Storing 40Gb in directory hidden from user (.varcoin) is generally bad idea

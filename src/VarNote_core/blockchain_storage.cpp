...
#include "VarNote_format_utils.h"
...
template<class Archive> void VarNote::blockchain_storage::TransactionIndex::serialize(Archive& archive, unsigned int version) {
  archive & block;
  archive & transaction;
}
...
bool blockchain_storage::init(const std::string& config_folder, bool load_existing, bool testnet) {
  ...
  if (!m_blocks.open(appendPath(config_folder, VarNote_BLOCKS_FILENAME), appendPath(config_folder, VarNote_BLOCKINDEXES_FILENAME), 1024)) {
    return false;
  }
  ...
  if (load_existing) {
    ...
    if (m_blocks.empty()) {
      ...
      bool rebuild = true;
      try {
        std::ifstream file(appendPath(config_folder, VarNote_BLOCKSCACHE_FILENAME), std::ios::binary);
...
bool blockchain_storage::store_blockchain() {
  try {
    std::ofstream file(appendPath(m_config_folder, VarNote_BLOCKSCACHE_FILENAME), std::ios::binary);
...
bool blockchain_storage::prevalidate_miner_transaction(const block& b, uint64_t height) {
  ...
  CHECK_AND_ASSERT_MES(b.miner_tx.unlock_time == height + VarNote_MINED_MONEY_UNLOCK_WINDOW,
    false,
    "coinbase transaction transaction have wrong unlock time=" << b.miner_tx.unlock_time << ", expected " << height + VarNote_MINED_MONEY_UNLOCK_WINDOW);
...
bool blockchain_storage::validate_miner_transaction(const block& b, size_t cumulative_block_size, uint64_t fee, uint64_t& base_reward, uint64_t already_generated_coins) {
  ...
  get_last_n_blocks_sizes(last_blocks_sizes, VarNote_REWARD_BLOCKS_WINDOW);
...
size_t blockchain_storage::find_end_of_allowed_index(const std::vector<std::pair<TransactionIndex, uint16_t>>& amount_outs) {
  ...
  do {
    --i;
    if (amount_outs[i].first.block + VarNote_MINED_MONEY_UNLOCK_WINDOW <= get_current_blockchain_height()) {
      return i + 1;
    }
  } while (i != 0);
...
bool blockchain_storage::is_tx_spendtime_unlocked(uint64_t unlock_time) {
  if (unlock_time < VarNote_MAX_BLOCK_NUMBER) {
    ...
    if (get_current_blockchain_height() - 1 + VarNote_LOCKED_TX_ALLOWED_DELTA_BLOCKS >= unlock_time)
      return true;
    else
      return false;
  } else {
...
bool blockchain_storage::check_block_timestamp_main(const block& b) {
  if (b.timestamp > get_adjusted_time() + VarNote_BLOCK_FUTURE_TIME_LIMIT) {
...
bool blockchain_storage::update_next_comulative_size_limit() {
  ...
  get_last_n_blocks_sizes(sz, VarNote_REWARD_BLOCKS_WINDOW);

  uint64_t median = epee::misc_utils::median(sz);
  if (median <= VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE)
    median = VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE;

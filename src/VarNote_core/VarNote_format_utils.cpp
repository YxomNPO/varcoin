...
namespace VarNote
{
  ...
  bool parse_amount(uint64_t& amount, const std::string& str_amount_)
  {
    ...
    if (std::string::npos != point_index)
    {
      ...
      while (VarNote_DISPLAY_DECIMAL_POINT < fraction_size && '0' == str_amount.back())
      {
        str_amount.erase(str_amount.size() - 1, 1);
        --fraction_size;
      }
      if (VarNote_DISPLAY_DECIMAL_POINT < fraction_size)
        return false;
    ...
    if (fraction_size < VarNote_DISPLAY_DECIMAL_POINT)
    {
      str_amount.append(VarNote_DISPLAY_DECIMAL_POINT - fraction_size, '0');
    }
  ...
  std::string print_money(uint64_t amount)
  {
    ...
    if(s.size() < VarNote_DISPLAY_DECIMAL_POINT+1)
    {
      s.insert(0, VarNote_DISPLAY_DECIMAL_POINT+1 - s.size(), '0');
    }
    s.insert(s.size() - VarNote_DISPLAY_DECIMAL_POINT, ".");
    return s;
  }
  //---------------------------------------------------------------
  ...
  bool generateTestnetGenesisBlock(VarNote::block& b) {

...
namespace VarNote
{
  ...
  //-----------------------------------------------------------------------------------------------
  core::core(i_VarNote_protocol* pprotocol):
  ...
  {
    set_VarNote_protocol(pprotocol);
  }
  void core::set_VarNote_protocol(i_VarNote_protocol* pprotocol)
  {
    if(pprotocol)
  ...
  bool core::check_tx_semantic(const transaction& tx, bool keeped_by_block)
  {
    ...
    if(!keeped_by_block && get_object_blobsize(tx) >= m_blockchain_storage.get_current_comulative_blocksize_limit() - VarNote_COINBASE_BLOB_RESERVED_SIZE)
    {
      LOG_PRINT_RED_L0("tx have to big size " << get_object_blobsize(tx) << ", expected not bigger than " << m_blockchain_storage.get_current_comulative_blocksize_limit() - VarNote_COINBASE_BLOB_RESERVED_SIZE);
      return false;
    }
  ...
  bool core::handle_block_found(block& b)
  {
    ...
    {
      VarNote_connection_context exclude_context = boost::value_initialized<VarNote_connection_context>();
  ...
  //-----------------------------------------------------------------------------------------------
  bool core::handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp, VarNote_connection_context& context)
  {

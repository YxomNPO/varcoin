...
namespace VarNote
{
  ...
  bool tx_memory_pool::fill_block_template(block &bl, size_t median_size, uint64_t already_generated_coins, size_t &total_size, uint64_t &fee)
  {
    ...
    size_t max_total_size = 2 * median_size - VarNote_COINBASE_BLOB_RESERVED_SIZE;
  ...
  bool tx_memory_pool::init(const std::string& config_folder)
  {
    ...
    std::string state_file_path = config_folder + "/" + VarNote_POOLDATA_FILENAME;
  ...
  bool tx_memory_pool::deinit()
  {
    ...
    std::string state_file_path = m_config_folder + "/" + VarNote_POOLDATA_FILENAME;

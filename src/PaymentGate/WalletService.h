...
public:
  ...
  explicit WalletService(const VarNote::Currency& currency, System::Dispatcher& sys, VarNote::INode& node, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  void fillTransactionRpcInfo(size_t txId, const VarNote::WalletTransaction& tx, TransactionRpcInfo& rpcInfo);
  void makeTransfers(const std::vector<TransferDestination>& destinations, std::vector<VarNote::WalletTransfer>& transfers);
  ...
  std::unique_ptr<VarNote::IWallet > wallet;
  VarNote::INode* node;

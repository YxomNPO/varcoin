...
class WalletService {
public:
  WalletService(const VarNote::Currency& currency, System::Dispatcher& sys, VarNote::INode& node, VarNote::IWallet& wallet,
    VarNote::IFusionManager& fusionManager, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  std::vector<VarNote::TransactionsInBlockInfo> getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const;
  std::vector<VarNote::TransactionsInBlockInfo> getTransactions(uint32_t firstBlockIndex, size_t blockCount) const;
  ...
  VarNote::IWallet& wallet;
  VarNote::IFusionManager& fusionManager;
  VarNote::INode& node;

...
void saveWallet(VarNote::IWallet* wallet, std::fstream& walletFile, bool saveDetailed = true, bool saveCache = true) {
  wallet->save(walletFile, saveDetailed, saveCache);
  walletFile.flush();
}

void secureSaveWallet(VarNote::IWallet* wallet, const std::string& path, bool saveDetailed = true, bool saveCache = true) {
...
void generateNewWallet(const VarNote::Currency &currency, const WalletConfiguration &conf, Logging::ILogger& logger, System::Dispatcher& dispatcher) {
  ...
  VarNote::IWallet* wallet = WalletFactory::createWallet(currency, *nodeStub, dispatcher);
...
void importLegacyKeys(const std::string &legacyKeysFile, const WalletConfiguration &conf) {
  ...
  VarNote::importLegacyKeys(legacyKeysFile, conf.walletPassword, archive);
...
void WalletService::loadPaymentsCacheAndTransferIndices() {
  ...
  for (size_t id = 0; id < txCount; ++id) {
    VarNote::WalletTransaction tx = wallet->getTransaction(id);
    ...
    if (!VarNote::getPaymentIdFromTxExtra(extraVector, paymentId)) {
    ...
    insertTransaction(id, paymentId, tx.blockHeight != VarNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT);
  }
}
...
std::error_code WalletService::sendTransaction(const SendTransactionRequest& req, SendTransactionResponse& resp) {
  ...
  try {
    ...
    if (txId == VarNote::WALLET_INVALID_TRANSACTION_ID) {
...
void WalletService::makeTransfers(const std::vector<PaymentService::TransferDestination>& destinations, std::vector<VarNote::WalletTransfer>& transfers) {
...
void WalletService::fillTransactionRpcInfo(size_t txId, const VarNote::WalletTransaction& tx, TransactionRpcInfo& rpcInfo) {
...
void WalletService::refresh() {
  try {
    for (;;) {
      ...
      if (event.type == VarNote::TRANSACTION_CREATED || event.type == VarNote::TRANSACTION_UPDATED) {
        ...
        insertTransaction(transactionId, paymentId, tx.blockHeight != VarNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT);

...
Crypto::Hash parsePaymentId(const std::string& paymentIdStr) {
  if (!checkPaymentId(paymentIdStr)) {
    throw std::system_error(make_error_code(VarNote::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT));
  }
...
bool getPaymentIdFromExtra(const std::string& binaryString, Crypto::Hash& paymentId) {
  return VarNote::getPaymentIdFromTxExtra(Common::asBinaryArray(binaryString), paymentId);
}
...
struct TransactionsInBlockInfoFilter {
  ...
  bool checkTransaction(const VarNote::WalletTransactionWithTransfers& transaction) const {
    if (havePaymentId) {
    ...
    for (const VarNote::WalletTransfer& transfer: transaction.transfers) {
...
void addPaymentIdToExtra(const std::string& paymentId, std::string& extra) {
  ...
  if (!VarNote::createTxExtraWithPaymentId(paymentId, extraVector)) {
    throw std::system_error(make_error_code(VarNote::error::BAD_PAYMENT_ID));
  }
...
Crypto::Hash parseHash(const std::string& hashString, Logging::LoggerRef logger) {
  ...
  if (!Common::podFromHex(hashString, hash)) {
    ...
    throw std::system_error(make_error_code(VarNote::error::WalletServiceErrorCode::WRONG_HASH_FORMAT));
  }

  return hash;
}

std::vector<VarNote::TransactionsInBlockInfo> filterTransactions(
  const std::vector<VarNote::TransactionsInBlockInfo>& blocks,
  const TransactionsInBlockInfoFilter& filter) {

  std::vector<VarNote::TransactionsInBlockInfo> result;

  for (const auto& block: blocks) {
    VarNote::TransactionsInBlockInfo item;
    item.blockHash = block.blockHash;

    for (const auto& transaction: block.transactions) {
      if (transaction.transaction.state != VarNote::WalletTransactionState::DELETED && filter.checkTransaction(transaction)) {
...
PaymentService::TransactionRpcInfo convertTransactionWithTransfersToTransactionRpcInfo(
  const VarNote::WalletTransactionWithTransfers& transactionWithTransfers) {
  ...
  for (const VarNote::WalletTransfer& transfer: transactionWithTransfers.transfers) {
...
std::vector<PaymentService::TransactionsInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionsInBlockRpcInfo(
  ...
  for (const auto& block: blocks) {
    ...
    for (const VarNote::WalletTransactionWithTransfers& transactionWithTransfers: block.transactions) {
...
std::vector<PaymentService::TransactionHashesInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(
  ...
  for (const VarNote::TransactionsInBlockInfo& block: blocks) {
    PaymentService::TransactionHashesInBlockRpcInfo item;
    item.blockHash = Common::podToHex(block.blockHash);

    for (const VarNote::WalletTransactionWithTransfers& transaction: block.transactions) {
...
void validateAddresses(const std::vector<std::string>& addresses, const VarNote::Currency& currency, Logging::LoggerRef logger) {
  for (const auto& address: addresses) {
    if (!VarNote::validateAddress(address, currency)) {
...
std::string getValidatedTransactionExtraString(const std::string& extraString) {
  ...
  if (!Common::fromHex(extraString, binary)) {
    throw std::system_error(make_error_code(VarNote::error::BAD_TRANSACTION_EXTRA));
  }
...
std::vector<VarNote::WalletOrder> convertWalletRpcOrdersToWalletOrders(const std::vector<PaymentService::WalletRpcOrder>& orders) {
  std::vector<VarNote::WalletOrder> result;
  result.reserve(orders.size());

  for (const auto& order: orders) {
    result.emplace_back(VarNote::WalletOrder {order.address, order.amount});
  }
...
void generateNewWallet(const VarNote::Currency& currency, const WalletConfiguration& conf, Logging::ILogger& logger, System::Dispatcher& dispatcher) {
  Logging::LoggerRef log(logger, "generateNewWallet");

  VarNote::INode* nodeStub = NodeFactory::createNodeStub();
  std::unique_ptr<VarNote::INode> nodeGuard(nodeStub);

  VarNote::IWallet* wallet = new VarNote::WalletGreen(dispatcher, currency, *nodeStub, logger);
  std::unique_ptr<VarNote::IWallet> walletGuard(wallet);
  ...
  wallet->save(VarNote::WalletSaveLevel::SAVE_KEYS_ONLY);
  log(Logging::INFO, Logging::BRIGHT_WHITE) << "Wallet is saved";
}

WalletService::WalletService(const VarNote::Currency& currency, System::Dispatcher& sys, VarNote::INode& node,
  VarNote::IWallet& wallet, VarNote::IFusionManager& fusionManager, const WalletConfiguration& conf, Logging::ILogger& logger) :
    currency(currency),
    wallet(wallet),
...
std::error_code WalletService::saveWalletNoThrow() {
  try {
    ...
    if (!inited) {
      ...
      return make_error_code(VarNote::error::NOT_INITIALIZED);
    }
    ...
    logger(Logging::WARNING, Logging::BRIGHT_YELLOW) << "Error while saving wallet: " << x.what();
    return make_error_code(VarNote::error::INTERNAL_WALLET_ERROR);
  }
...
std::error_code WalletService::replaceWithNewWallet(const std::string& viewSecretKeyText) {
  try {
    ...
    if (!Common::podFromHex(viewSecretKeyText, viewSecretKey)) {
      ...
      return make_error_code(VarNote::error::WalletServiceErrorCode::WRONG_KEY_FORMAT);
    }
  ...
  try {
    ...
    for (auto& keyText : spendSecretKeysText) {
      ...
      if (!insertResult.second) {
        ...
        return make_error_code(VarNote::error::WalletServiceErrorCode::DUPLICATE_KEY);
      }
...
std::error_code WalletService::getSpendkeys(const std::string& address, std::string& publicSpendKeyText, std::string& secretSpendKeyText) {
  try {
    ...
    VarNote::KeyPair key = wallet.getAddressSpendKey(address);
...
std::error_code WalletService::getViewKey(std::string& viewSecretKey) {
  try {
    System::EventLock lk(readyEvent);
    VarNote::KeyPair viewKey = wallet.getViewKey();
...
std::error_code WalletService::getTransaction(const std::string& transactionHash, TransactionRpcInfo& transaction) {
  try {
    ...
    VarNote::WalletTransactionWithTransfers transactionWithTransfers = wallet.getTransaction(hash);

    if (transactionWithTransfers.transaction.state == VarNote::WalletTransactionState::DELETED) {
      logger(Logging::WARNING, Logging::BRIGHT_YELLOW) << "Transaction " << transactionHash << " is deleted";
      return make_error_code(VarNote::error::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::sendTransaction(const SendTransaction::Request& request, std::string& transactionHash) {
  try {
    ...
    VarNote::TransactionParameters sendParams;
...
std::error_code WalletService::deleteDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    if (idIt == transactionIdIndex.end()) {
      return make_error_code(VarNote::error::WalletServiceErrorCode::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::getUnconfirmedTransactionHashes(const std::vector<std::string>& addresses, std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::vector<VarNote::WalletTransactionWithTransfers> transactions = wallet.getUnconfirmedTransactions();
...
void WalletService::refresh() {
  try {
    ...
    for (;;) {
      ...
      if (event.type == VarNote::TRANSACTION_CREATED) {
...
std::vector<VarNote::TransactionsInBlockInfo> WalletService::getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const {
  std::vector<VarNote::TransactionsInBlockInfo> result = wallet.getTransactions(blockHash, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(VarNote::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }

  return result;
}

std::vector<VarNote::TransactionsInBlockInfo> WalletService::getTransactions(uint32_t firstBlockIndex, size_t blockCount) const {
  std::vector<VarNote::TransactionsInBlockInfo> result = wallet.getTransactions(firstBlockIndex, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(VarNote::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(const Crypto::Hash& blockHash, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<VarNote::TransactionsInBlockInfo> allTransactions = getTransactions(blockHash, blockCount);
  std::vector<VarNote::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(uint32_t firstBlockIndex, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<VarNote::TransactionsInBlockInfo> allTransactions = getTransactions(firstBlockIndex, blockCount);
  std::vector<VarNote::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
  return convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(filteredTransactions);
}

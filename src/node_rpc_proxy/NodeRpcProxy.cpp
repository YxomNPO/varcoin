...
void NodeRpcProxy::getNewBlocks(std::list<crypto::hash>&& knownBlockIds, std::list<VarNote::block_complete_entry>& newBlocks, uint64_t& startHeight, const Callback& callback) {
  if (!m_initState.initialized()) {
    callback(make_error_code(error::NOT_INITIALIZED));
    return;
  }
...
void NodeRpcProxy::doRelayTransaction(const VarNote::transaction& transaction, const Callback& callback) {
  COMMAND_RPC_SEND_RAW_TX::request req;
  COMMAND_RPC_SEND_RAW_TX::response rsp;
  req.tx_as_hex = epee::string_tools::buff_to_hex_nodelimer(VarNote::tx_to_blob(transaction));
...
void NodeRpcProxy::doGetNewBlocks(std::list<crypto::hash>& knownBlockIds, std::list<VarNote::block_complete_entry>& newBlocks, uint64_t& startHeight, const Callback& callback) {

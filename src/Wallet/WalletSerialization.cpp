...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const VarNote::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  VarNote::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  WalletTransferDto() {}
  WalletTransferDto(const VarNote::WalletTransfer& tr) {
    address = tr.address;
    amount = tr.amount;
  }
...
void serialize(WalletRecordDto& value, VarNote::ISerializer& serializer) {
...
void serialize(SpentOutputDto& value, VarNote::ISerializer& serializer) {
...
void serialize(ChangeDto& value, VarNote::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, VarNote::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, VarNote::ISerializer& serializer) {
  typedef std::underlying_type<VarNote::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<VarNote::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  VarNote::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, VarNote::ISerializer& serializer) {
...
std::string serialize(Object& obj, const std::string& name) {
  ...
  VarNote::BinaryOutputStreamSerializer s(output);
...
std::string encrypt(const std::string& plain, VarNote::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  VarNote::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, VarNote::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  VarNote::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, VarNote::CryptoContext& cryptoContext) {
  std::string plain;
...
void deserialize(Object& obj, const std::string& name, const std::string& plain) {
  ...
  VarNote::BinaryInputStreamSerializer s(stream);
  s(obj, Common::StringView(name));
}

template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, VarNote::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
VarNote::WalletTransaction convert(const VarNote::WalletLegacyTransaction& tx) {
  VarNote::WalletTransaction mtx;

  mtx.state = VarNote::WalletTransactionState::SUCCEEDED;
...
VarNote::WalletTransfer convert(const VarNote::WalletLegacyTransfer& tr) {
  VarNote::WalletTransfer mtr;
...
void WalletSerializer::loadCurrentVersion(Common::IInputStream& source, const std::string& password) {
  VarNote::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  VarNote::BinaryInputStreamSerializer encrypted(source);
  ...
  VarNote::BinaryInputStreamSerializer serializer(decryptedStream);
...
void WalletSerializer::loadWalletV1Keys(VarNote::BinaryInputStreamSerializer& serializer) {
  VarNote::KeysStorage keys;
...
void WalletSerializer::loadWalletV1Details(VarNote::BinaryInputStreamSerializer& serializer) {

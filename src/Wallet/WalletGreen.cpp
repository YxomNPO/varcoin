...
void parseAddressString(const std::string& string, const VarNote::Currency& currency, VarNote::AccountPublicAddress& address) {
...
void validateAddresses(const std::vector<VarNote::WalletTransfer>& destinations, const VarNote::Currency& currency) {
...
uint64_t countNeededMoney(const std::vector<VarNote::WalletTransfer>& destinations, uint64_t fee) {
...
void checkIfEnoughMixins(std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint64_t mixIn) {
...
bool WalletGreen::updateWalletTransactionInfo(const Hash& hash, const VarNote::TransactionInformation& info) {
  auto& hashIndex = m_transactions.get<TransactionIndex>();
...
void WalletGreen::sendTransaction(ITransaction* tx) {
  ...
  std::error_code ec;
  VarNote::Transaction oldTxFormat;
...
void WalletGreen::splitDestinations(const std::vector<VarNote::WalletTransfer>& destinations,
  const VarNote::WalletTransfer& changeDestination,
  uint64_t dustThreshold,
...
void WalletGreen::transactionUpdated(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  VarNote::TransactionInformation info;
  int64_t txBalance;
  ...
  if (info.blockHeight != VarNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT) {
...
void WalletGreen::transactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  m_transactions.get<TransactionIndex>().modify(it, [] (VarNote::WalletTransaction& tx) {

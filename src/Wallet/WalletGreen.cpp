...
VarNote::WalletEvent makeTransactionUpdatedEvent(size_t id) {
  VarNote::WalletEvent event;
  event.type = VarNote::WalletEventType::TRANSACTION_UPDATED;
  event.transactionUpdated.transactionIndex = id;

  return event;
}

VarNote::WalletEvent makeTransactionCreatedEvent(size_t id) {
  VarNote::WalletEvent event;
  event.type = VarNote::WalletEventType::TRANSACTION_CREATED;
  event.transactionCreated.transactionIndex = id;

  return event;
}

VarNote::WalletEvent makeMoneyUnlockedEvent() {
  VarNote::WalletEvent event;
  event.type = VarNote::WalletEventType::BALANCE_UNLOCKED;

  return event;
}

VarNote::WalletEvent makeSyncProgressUpdatedEvent(uint32_t current, uint32_t total) {
  VarNote::WalletEvent event;
  event.type = VarNote::WalletEventType::SYNC_PROGRESS_UPDATED;
...
VarNote::WalletEvent makeSyncCompletedEvent() {
  VarNote::WalletEvent event;
  event.type = VarNote::WalletEventType::SYNC_COMPLETED;
...
{
  m_upperTransactionSizeLimit = parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_CURRENT * 125 / 100 - m_currency.minerTxBlobReservedSize();
  m_readyEvent.set();
}
...
void WalletGreen::initializeWithViewKey(const std::string& path, const std::string& password, const Crypto::SecretKey& viewSecretKey) {
  ...
  if (!Crypto::secret_key_to_public_key(viewSecretKey, viewPublicKey)) {
    ...
    throw std::system_error(make_error_code(VarNote::error::KEY_GENERATION_ERROR));
  }
  ...
  if (clearCachedData) {
    ...
    for (auto it = m_walletsContainer.begin(); it != m_walletsContainer.end(); ++it) {
      m_walletsContainer.modify(it, [&walletIndex](WalletRecord& wallet) {
        ...
        wallet.container = reinterpret_cast<VarNote::ITransfersContainer*>(walletIndex++); //dirty hack. container field must be unique
      });
    }
...
void WalletGreen::initWithKeys(const std::string& path, const std::string& password,
  ...
  if (m_state != WalletState::NOT_INITIALIZED) {
    ...
    throw std::system_error(make_error_code(VarNote::error::ALREADY_INITIALIZED));
  }
...
void WalletGreen::loadSpendKeys() {
  ...
  for (size_t i = 0; i < m_containerStorage.size(); ++i) {
    ...
    wallet.actualBalance = 0;
    wallet.pendingBalance = 0;
    wallet.container = reinterpret_cast<VarNote::ITransfersContainer*>(i); //dirty hack. container field must be unique
...
KeyPair WalletGreen::getAddressSpendKey(const std::string& address) const {
  ...
  VarNote::AccountPublicAddress pubAddr = parseAddress(address);
...
WalletTransaction WalletGreen::getTransaction(size_t transactionIndex) const {
  ...
  if (m_transactions.size() <= transactionIndex) {
    ...
    throw std::system_error(make_error_code(VarNote::error::INDEX_OUT_OF_RANGE));
  }
...
void WalletGreen::prepareTransaction(std::vector<WalletOuts>&& wallets,
  ...
  const VarNote::AccountPublicAddress& changeDestination,
  ...
  typedef VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount outs_for_amount;
...
void WalletGreen::checkIfEnoughMixins(std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint16_t mixIn) const {
  assert(mixIn != 0);

  auto notEnoughIt = std::find_if(mixinResult.begin(), mixinResult.end(),
    [mixIn](const VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount& ofa) { return ofa.outs.size() < mixIn; });

  if (notEnoughIt != mixinResult.end()) {
    m_logger(ERROR, BRIGHT_RED) << "Mixin is too big: " << mixIn;
    throw std::system_error(make_error_code(VarNote::error::MIXIN_COUNT_TOO_BIG));
  }
}
...
std::vector<WalletTransfer> WalletGreen::convertOrdersToTransfers(const std::vector<WalletOrder>& orders) const {
  ...
  for (const auto& order : orders) {
    ...
    if (order.amount > static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
      ...
      throw std::system_error(make_error_code(VarNote::error::WRONG_AMOUNT), message);
    }
...
uint64_t WalletGreen::countNeededMoney(const std::vector<VarNote::WalletTransfer>& destinations, uint64_t fee) const {
  ...
  for (const auto& transfer : destinations) {
    if (transfer.amount == 0) {
      ...
      throw std::system_error(make_error_code(VarNote::error::ZERO_DESTINATION));
    ...
    if (neededMoney <= std::numeric_limits<uint64_t>::max() - uamount) {
      ...
      throw std::system_error(make_error_code(VarNote::error::SUM_OVERFLOW));
    }
  }
...
VarNote::AccountPublicAddress WalletGreen::parseAccountAddressString(const std::string& addressString) const {
  VarNote::AccountPublicAddress address;

  if (!m_currency.parseAccountAddressString(addressString, address)) {
    m_logger(ERROR, BRIGHT_RED) << "Bad address: " << addressString;
    throw std::system_error(make_error_code(VarNote::error::BAD_ADDRESS));
  }
...
void WalletGreen::validateAddresses(const std::vector<std::string>& addresses) const {
  for (const auto& address : addresses) {
    if (!VarNote::validateAddress(address, m_currency)) {
...
void WalletGreen::validateOrders(const std::vector<WalletOrder>& orders) const {
  for (const auto& order : orders) {
    if (!VarNote::validateAddress(order.address, m_currency)) {
...
void WalletGreen::validateChangeDestination(const std::vector<std::string>& sourceAddresses, const std::string& changeDestination, bool isFusion) const {
  ...
  if (changeDestination.empty()) {
    ...
    if (!VarNote::validateAddress(changeDestination, m_currency)) {
      message = std::string("Bad ") + (isFusion ? "destination" : "change destination") + " address: " + changeDestination;
      m_logger(ERROR, BRIGHT_RED) << message;
      throw std::system_error(make_error_code(VarNote::error::BAD_ADDRESS), message);
    }
...
size_t WalletGreen::doTransfer(const TransactionParameters& transactionParameters) {
  ...
  VarNote::AccountPublicAddress changeDestination = getChangeDestination(transactionParameters.changeDestination, transactionParameters.sourceAddresses);
...
size_t WalletGreen::makeTransaction(const TransactionParameters& sendingTransaction) {
  ...
  VarNote::AccountPublicAddress changeDestination = getChangeDestination(sendingTransaction.changeDestination, sendingTransaction.sourceAddresses);
  m_logger(DEBUGGING) << "Change address " << m_currency.accountAddressAsString(changeDestination);
...
size_t WalletGreen::insertOutgoingTransactionAndPushEvent(const Hash& transactionHash, uint64_t fee, const BinaryArray& extra, uint64_t unlockTimestamp) {
  ...
  insertTx.blockHeight = VarNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT;
...
bool WalletGreen::updateWalletTransactionInfo(size_t transactionId, const VarNote::TransactionInformation& info, int64_t totalAmount) {
...
std::unique_ptr<VarNote::ITransaction> WalletGreen::makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
...
void WalletGreen::sendTransaction(const VarNote::Transaction& varNoteTransaction) {
  System::Event completion(m_dispatcher);
  std::error_code ec;

  throwIfStopped();

  System::RemoteContext<void> relayTransactionContext(m_dispatcher, [this, &varNoteTransaction, &ec, &completion] () {
    m_node.relayTransaction(varNoteTransaction, [&ec, &completion, this](std::error_code error) {
      ec = error;
      this->m_dispatcher.remoteSpawn(std::bind(asyncRequestCompletion, std::ref(completion)));
    });
  });
  ...
  if (ec) {
    m_logger(ERROR, BRIGHT_RED) << "Failed to relay transaction: " << ec << ", " << ec.message() <<
      ". Transaction hash " << getObjectHash(varNoteTransaction);
    throw std::system_error(ec);
  }
}
...
size_t WalletGreen::validateSaveAndSendTransaction(const ITransactionReader& transaction, const std::vector<WalletTransfer>& destinations, bool isFusion, bool send) {
  ...
  VarNote::Transaction varNoteTransaction;
  if (!fromBinaryArray(varNoteTransaction, transactionData)) {
  ...
  if (send) {
    sendTransaction(varNoteTransaction);
    ...
    m_uncommitedTransactions.emplace(transactionId, std::move(varNoteTransaction));
...
void WalletGreen::requestMixinOuts(
  ...
  uint16_t mixIn,
  std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult) {
...
std::vector<VarNote::WalletGreen::ReceiverAmounts> WalletGreen::splitDestinations(const std::vector<VarNote::WalletTransfer>& destinations,
  uint64_t dustThreshold,
...
VarNote::WalletGreen::ReceiverAmounts WalletGreen::splitAmount(
  uint64_t amount,
...
void WalletGreen::prepareInputs(
  ...
  typedef VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry out_entry;
...
void WalletGreen::throwIfNotInitialized() const {
  if (m_state != WalletState::INITIALIZED) {
    ...
    throw std::system_error(make_error_code(VarNote::error::NOT_INITIALIZED));
  }
}
...
void WalletGreen::transactionUpdated(const TransactionInformation& transactionInfo, const std::vector<ContainerAmounts>& containerAmountsList) {
  ...
  if (transactionInfo.blockHeight != VarNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT) {
...
void WalletGreen::transactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  VarNote::ITransfersContainer* container = &object->getContainer();
  updateBalance(container);
  deleteUnlockTransactionJob(transactionHash);

  bool updated = false;
  m_transactions.get<TransactionIndex>().modify(it, [this, &transactionHash, &updated](VarNote::WalletTransaction& tx) {
...
void WalletGreen::insertUnlockTransactionJob(const Hash& transactionHash, uint32_t blockHeight, VarNote::ITransfersContainer* container) {
  auto& index = m_unlockTransactionsJob.get<BlockHeightIndex>();
...
void WalletGreen::updateBalance(VarNote::ITransfersContainer* container) {
...
const WalletRecord& WalletGreen::getWalletRecord(VarNote::ITransfersContainer* container) const {
  auto it = m_walletsContainer.get<TransfersContainerIndex>().find(container);
  if (it == m_walletsContainer.get<TransfersContainerIndex>().end()) {
...
VarNote::AccountPublicAddress WalletGreen::parseAddress(const std::string& address) const {
...
VarNote::AccountPublicAddress WalletGreen::getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const {
...
bool WalletGreen::isMyAddress(const std::string& addressString) const {
  VarNote::AccountPublicAddress address = parseAccountAddressString(addressString);

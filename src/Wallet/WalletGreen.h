...
protected:
  ...
  struct ReceiverAmounts {
    VarNote::AccountPublicAddress receiver;
...
#pragma pack(pop)
  ...
  void updateBalance(VarNote::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(VarNote::ITransfersContainer* container) const;

  VarNote::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const VarNote::AccountPublicAddress& changeDestinationAddress,
    PreparedTransaction& preparedTransaction);

  size_t doTransfer(const TransactionParameters& transactionParameters);

  void checkIfEnoughMixins(std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint16_t mixIn) const;
  std::vector<WalletTransfer> convertOrdersToTransfers(const std::vector<WalletOrder>& orders) const;
  uint64_t countNeededMoney(const std::vector<VarNote::WalletTransfer>& destinations, uint64_t fee) const;
  VarNote::AccountPublicAddress parseAccountAddressString(const std::string& addressString) const;
  ...
  std::unique_ptr<VarNote::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const VarNote::Transaction& varNoteTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const VarNote::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, VarNote::ITransfersContainer* container);
  ...
  VarNote::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
  ...
  friend std::ostream& operator<<(std::ostream& os, VarNote::WalletGreen::WalletState state);
  friend std::ostream& operator<<(std::ostream& os, VarNote::WalletGreen::WalletTrackingMode mode);

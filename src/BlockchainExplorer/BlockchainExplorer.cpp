...
bool BlockchainExplorer::addObserver(IBlockchainObserver* observer) {
  if (state.load() != INITIALIZED) {
    throw std::system_error(make_error_code(VarNote::error::BlockchainExplorerErrorCodes::NOT_INITIALIZED));
  }
...
void BlockchainExplorer::init() {
  if (state.load() != NOT_INITIALIZED) {
    ...
    throw std::system_error(make_error_code(VarNote::error::BlockchainExplorerErrorCodes::ALREADY_INITIALIZED));
  }

  if (!getBlockchainTop(knownBlockchainTop, false)) {
    logger(ERROR) << "Can't get blockchain top.";
    throw std::system_error(make_error_code(VarNote::error::BlockchainExplorerErrorCodes::INTERNAL_ERROR));
  }
...
bool BlockchainExplorer::getBlocks(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<BlockDetails>& blocks, uint32_t& blocksNumberWithinTimestamps) {
  ...
  if (timestampBegin > timestampEnd) {
    throw std::system_error(make_error_code(VarNote::error::BlockchainExplorerErrorCodes::REQUEST_ERROR), "timestampBegin must not be greater than timestampEnd");
  }
...
uint64_t BlockchainExplorer::getRewardBlocksWindow() {
  ...
  return parameters::VARNOTE_REWARD_BLOCKS_WINDOW;
}
...
uint64_t BlockchainExplorer::getFullRewardMaxBlockSize(uint8_t majorVersion) {
  ...
  if (majorVersion >= BLOCK_MAJOR_VERSION_3) {
    return parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE;
  } else if (majorVersion == BLOCK_MAJOR_VERSION_2) {
    return parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V2;
  } else {
    return parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1;
  }
}

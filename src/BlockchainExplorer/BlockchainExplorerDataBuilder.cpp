...
BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(VarNote::ICore& core, VarNote::IVarNoteProtocolQuery& protocol) :
core(core),
protocol(protocol) {
}
...
bool BlockchainExplorerDataBuilder::fillBlockDetails(const Block &block, BlockDetails& blockDetails) {
  ...
  if (!core.getBackwardBlocksSizes(blockDetails.height, blocksSizes, parameters::VARNOTE_REWARD_BLOCKS_WINDOW)) {
    return false;
  }

...
#include "VarNoteSerialization.h"
...
struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const VarNote::BaseInput) { return  0xff; }
  uint8_t operator()(const VarNote::KeyInput) { return  0x2; }
  uint8_t operator()(const VarNote::MultisignatureInput) { return  0x3; }
  uint8_t operator()(const VarNote::KeyOutput) { return  0x2; }
  uint8_t operator()(const VarNote::MultisignatureOutput) { return  0x3; }
  uint8_t operator()(const VarNote::Transaction) { return  0xcc; }
  uint8_t operator()(const VarNote::BlockTemplate) { return  0xbb; }
};

struct VariantSerializer : boost::static_visitor<> {
  VariantSerializer(VarNote::ISerializer& serializer, const std::string& name) : s(serializer), name(name) {}

  template <typename T>
  void operator() (T& param) { s(param, name); }

  VarNote::ISerializer& s;
  std::string name;
};

void getVariantValue(VarNote::ISerializer& serializer, uint8_t tag, VarNote::TransactionInput& in) {
  switch(tag) {
  case 0xff: {
    VarNote::BaseInput v;
    serializer(v, "value");
...
void getVariantValue(VarNote::ISerializer& serializer, uint8_t tag, VarNote::TransactionOutputTarget& out) {
  switch(tag) {
  case 0x2: {
    VarNote::KeyOutput v;
    serializer(v, "data");
  ...
  case 0x3: {
    VarNote::MultisignatureOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
...
template <typename T>
bool serializePod(T& v, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializer.binary(&v, sizeof(v), name);
}

bool serializeVarintVector(std::vector<uint32_t>& vector, VarNote::ISerializer& serializer, Common::StringView name) {
...
bool serialize(PublicKey& pubKey, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(pubKey, name, serializer);
}

bool serialize(SecretKey& secKey, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(secKey, name, serializer);
}

bool serialize(Hash& h, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(h, name, serializer);
}

bool serialize(KeyImage& keyImage, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(keyImage, name, serializer);
}

bool serialize(chacha8_iv& chacha, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(chacha, name, serializer);
}

bool serialize(Signature& sig, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(sig, name, serializer);
}

bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializePod(ecScalar, name, serializer);
}

bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name, VarNote::ISerializer& serializer) {

...
#include "VarNoteCore/IMinerHandler.h"
...
namespace VarNote {
  ...
  class core : public ICore, public IMinerHandler, public IBlockchainStorageObserver, public ITxPoolObserver {
   public:
     core(const Currency& currency, i_varnote_protocol* pprotocol, Logging::ILogger& logger);
     ~core();

     bool on_idle();
     virtual bool handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc, bool keeped_by_block); //Deprecated. Should be removed with VarNoteProtocolHandler.
     ...
     virtual i_varnote_protocol* get_protocol(){return m_pprotocol;}
     ...
     virtual bool handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS_request& arg, NOTIFY_RESPONSE_GET_OBJECTS_request& rsp) override; //Deprecated. Should be removed with VarNoteProtocolHandler.
    ...
    virtual Crypto::Hash getBlockIdByHeight(uint32_t height) override;
     ...
     void set_varnote_protocol(i_varnote_protocol* pprotocol);
   ...
   private:
     ...
     VarNote::RealTimeProvider m_timeProvider;
     tx_memory_pool m_mempool;
     Blockchain m_blockchain;
     i_varnote_protocol* m_pprotocol;
     std::unique_ptr<miner> m_miner;
     std::string m_config_folder;
     varnote_protocol_stub m_protocol_stub;

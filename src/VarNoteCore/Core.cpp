...
#include "VarNoteCore/CoreErrors.h"
#include "VarNoteCore/MemoryBlockchainStorage.h"
#include "VarNoteCore/TransactionExtra.h"
#include "VarNoteCore/TransactionPool.h"
#include "VarNoteCore/TransactionPoolCleaner.h"
#include "VarNoteCore/UpgradeManager.h"
...
TransactionValidatorState extractSpentOutputs(const CachedTransaction& transaction) {
  ...
  const auto& varnoteTransaction = transaction.getTransaction();

  for (const auto& input : varnoteTransaction.inputs) {
    if (input.type() == typeid(KeyInput)) {
...
std::vector<Crypto::Hash> VarNote::Core::getBlockHashes(uint32_t startBlockIndex, uint32_t maxCount) const {
...
uint64_t VarNote::Core::getAdjustedTime() const {
  return time(NULL);
}

...
namespace VarNote {
  ...
  static_assert(VarNote::UpgradeDetectorBase::UNDEF_HEIGHT == UINT32_C(0xFFFFFFFF), "UpgradeDetectorBase::UNDEF_HEIGHT has invalid value");
      ...
      if (m_currency.upgradeHeight(m_targetVersion) != UNDEF_HEIGHT) {
        ...
        if (m_blockchain.size() <= upgradeHeight()) {
          ...
          if (m_blockchain.size() % (60 * 60 / m_currency.difficultyTarget()) == 0) {
            ...
            VarNote::CachedBlock cachedBlock(m_blockchain.back().bl);

...
bool Currency::generateGenesisBlock() {
  ...
  std::string genesisCoinbaseTxHex = "010a01ff0001fbe6df2f029b2e4c0281c0b02e7c53291a94d1d0cbff8883f8024f5142ee494ffbbd0880712101067f102abfe1f8d499220733fe38b3ddf93241ae4720936f113fe12961ca4bfe";
...
size_t Currency::blockGrantedFullRewardZoneByBlockVersion(uint8_t blockMajorVersion) const {
  if (blockMajorVersion >= BLOCK_MAJOR_VERSION_3) {
    ...
    return VarNote::parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V2;
  } else {
    return VarNote::parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1;
  }
}
...
bool Currency::parseAccountAddressString(const std::string& str, AccountPublicAddress& addr) const {
  uint64_t prefix;
  if (!VarNote::parseAccountAddressString(prefix, addr, str)) {
    return false;
  }
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  maxBlockNumber(parameters::VARNOTE_MAX_BLOCK_NUMBER);
  maxBlockBlobSize(parameters::VARNOTE_MAX_BLOCK_BLOB_SIZE);
  maxTxSize(parameters::VARNOTE_MAX_TX_SIZE);
  publicAddressBase58Prefix(parameters::VARNOTE_PUBLIC_ADDRESS_BASE58_PREFIX);
  minedMoneyUnlockWindow(parameters::VARNOTE_MINED_MONEY_UNLOCK_WINDOW);

  timestampCheckWindow(parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
  blockFutureTimeLimit(parameters::VARNOTE_BLOCK_FUTURE_TIME_LIMIT);

  moneySupply(parameters::MONEY_SUPPLY);
  emissionSpeedFactor(parameters::EMISSION_SPEED_FACTOR);

  rewardBlocksWindow(parameters::VARNOTE_REWARD_BLOCKS_WINDOW);
  blockGrantedFullRewardZone(parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE);
  minerTxBlobReservedSize(parameters::VARNOTE_COINBASE_BLOB_RESERVED_SIZE);

  numberOfDecimalPlaces(parameters::VARNOTE_DISPLAY_DECIMAL_POINT);
  ...
  lockedTxAllowedDeltaSeconds(parameters::VARNOTE_LOCKED_TX_ALLOWED_DELTA_SECONDS);
  lockedTxAllowedDeltaBlocks(parameters::VARNOTE_LOCKED_TX_ALLOWED_DELTA_BLOCKS);

  mempoolTxLiveTime(parameters::VARNOTE_MEMPOOL_TX_LIVETIME);
  mempoolTxFromAltBlockLiveTime(parameters::VARNOTE_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME);
  numberOfPeriodsToForgetTxDeletedFromPool(parameters::VARNOTE_NUMBER_OF_PERIODS_TO_FORGET_TX_DELETED_FROM_POOL);
  ...
  blocksFileName(parameters::VARNOTE_BLOCKS_FILENAME);
  blockIndexesFileName(parameters::VARNOTE_BLOCKINDEXES_FILENAME);
  txPoolFileName(parameters::VARNOTE_POOLDATA_FILENAME);

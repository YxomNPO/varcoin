...
namespace VarNote {
  ...
  public:
    ...
    TransactionImpl(const VarNote::Transaction& tx);
  ...
  std::unique_ptr<ITransaction> createTransaction(const VarNote::Transaction& tx) {
    return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
  }

  TransactionImpl::TransactionImpl() {   
    VarNote::KeyPair txKeys(VarNote::generateKeyPair());
  ...
  TransactionImpl::TransactionImpl(const VarNote::Transaction& tx) : transaction(tx) {
    extra.parse(transaction.extra);
  }
  ...
  bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress& addr, const SecretKey& viewSecretKey, std::vector<uint32_t>& out, uint64_t& amount) const {
    return ::VarNote::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
  }

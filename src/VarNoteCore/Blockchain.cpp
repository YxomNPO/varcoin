...
bool serialize(google::sparse_hash_map<K, V, Hash>& value, Common::StringView name, VarNote::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [&value](size_t size) { value.resize(size); });
}

template<typename K, typename Hash>
bool serialize(google::sparse_hash_set<K, Hash>& value, Common::StringView name, VarNote::ISerializer& serializer) {
...
bool serialize(std::vector<std::pair<Blockchain::TransactionIndex, uint16_t>>& value, Common::StringView name, VarNote::ISerializer& s) {
  ...
  if (s.type() == VarNote::ISerializer::INPUT) {
...
bool Blockchain::checkTransactionInputs(const VarNote::Transaction& tx, BlockInfo& maxUsedBlock) {
  return checkTransactionInputs(tx, maxUsedBlock.height, maxUsedBlock.id);
}

bool Blockchain::checkTransactionInputs(const VarNote::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) {
...
bool Blockchain::haveSpentKeyImages(const VarNote::Transaction& tx) {
...
bool Blockchain::handleGetObjects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with VarNoteProtocolHandler.
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
...
bool Blockchain::update_next_comulative_size_limit() {
  ...
  size_t nextBlockGrantedFullRewardZone = nextBlockMajorVersion == BLOCK_MAJOR_VERSION_1 ?
    parameters::VARNOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1 :

...
namespace VarNote
{
  ...
  bool miner::set_block_template(const Block& bl, const difficulty_type& di) {
    ...
    if (BLOCK_MAJOR_VERSION_2 == m_template.majorVersion) {
      VarNote::TransactionExtraMergeMiningTag mm_tag;
      mm_tag.depth = 0;
      if (!VarNote::get_aux_block_header_hash(m_template, mm_tag.merkleRoot)) {
        return false;
      }

      m_template.parentBlock.baseTransaction.extra.clear();
      if (!VarNote::appendMergeMiningTagToExtra(m_template.parentBlock.baseTransaction.extra, mm_tag)) {
        return false;
      }
    }
  ...
  bool miner::request_block_template() {
    ...
    uint32_t height;
    VarNote::BinaryArray extra_nonce;
  ...
  bool miner::init(const MinerConfig& config) {
    if (!config.extraMessages.empty()) {
      ...
      std::string filebuf;
      if (Common::loadFileToString(m_config_folder_path + "/" + VarNote::parameters::MINER_CONFIG_FILE_NAME, filebuf)) {
    ...
    while(!m_stop)
    {
      ...
      if (!m_stop && check_hash(h, local_diff))
      {
        ...
        if(!m_handler.handle_block_found(b)) {
          ...
          Common::saveStringToFile(m_config_folder_path + "/" + VarNote::parameters::MINER_CONFIG_FILE_NAME, storeToJson(m_config));
        }
      }

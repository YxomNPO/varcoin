...
void RocksDBWrapper::init(const DataBaseConfig& config) {
  if (state.load() != NOT_INITIALIZED) {
    throw std::system_error(make_error_code(VarNote::error::DataBaseErrorCodes::ALREADY_INITIALIZED));
  }
  ...
  if (status.ok()) {
    ...
    if (!status.ok()) {
      ...
      throw std::system_error(make_error_code(VarNote::error::DataBaseErrorCodes::INTERNAL_ERROR));
    }
  } else {
...
void RocksDBWrapper::shutdown() {
  if (state.load() != INITIALIZED) {
    throw std::system_error(make_error_code(VarNote::error::DataBaseErrorCodes::NOT_INITIALIZED));
  }
...
std::error_code RocksDBWrapper::write(IWriteBatch& batch, bool sync) {
  ...
  if (!status.ok()) {
    ...
    return make_error_code(VarNote::error::DataBaseErrorCodes::INTERNAL_ERROR);
  } else {

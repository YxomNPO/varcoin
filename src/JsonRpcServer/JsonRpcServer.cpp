...
void JsonRpcServer::processRequest(const VarNote::HttpRequest& req, VarNote::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(VarNote::HttpResponse::STATUS_200);
      ...
      resp.setStatus(VarNote::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(VarNote::HttpResponse::STATUS_500);
  }
}

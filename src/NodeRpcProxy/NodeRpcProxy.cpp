...
void NodeRpcProxy::resetInternalState() {
  ...
  m_lastKnowHash = VarNote::NULL_HASH;
  m_knownTxs.clear();
}
...
void NodeRpcProxy::updatePeerCount() {
  VarNote::COMMAND_RPC_GET_INFO::request req = AUTO_VAL_INIT(req);
  VarNote::COMMAND_RPC_GET_INFO::response rsp = AUTO_VAL_INIT(rsp);
...
void NodeRpcProxy::getNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds,
                                std::vector<VarNote::block_complete_entry>& newBlocks,

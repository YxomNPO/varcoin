...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  VarNote::core core(currency, NULL, logger);

  VarNote::VarNoteProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  VarNote::NodeServer p2pNode(*dispatcher, protocol, logger);

  protocol.set_p2p_endpoint(&p2pNode);
  core.set_varnote_protocol(&protocol);
  ...
  std::unique_ptr<VarNote::INode> node(new VarNote::InProcessNode(core, protocol));

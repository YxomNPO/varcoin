...
using VarNote::SerializationTag;

namespace {

struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const VarNote::BaseInputDetails) { return static_cast<uint8_t>(SerializationTag::Base); }
  uint8_t operator()(const VarNote::KeyInputDetails) { return static_cast<uint8_t>(SerializationTag::Key); }
  uint8_t operator()(const VarNote::MultisignatureInputDetails) { return static_cast<uint8_t>(SerializationTag::Multisignature); }
};
...
void getVariantValue(VarNote::ISerializer& serializer, uint8_t tag, boost::variant<VarNote::BaseInputDetails,
                                                                                      VarNote::KeyInputDetails,
                                                                                      VarNote::MultisignatureInputDetails>& in) {
  switch (static_cast<SerializationTag>(tag)) {
  case SerializationTag::Base: {
    VarNote::BaseInputDetails v;
    serializer(v, "data");
    in = v;
    break;
  }
  case SerializationTag::Key: {
    VarNote::KeyInputDetails v;
    serializer(v, "data");
    in = v;
    break;
  }
  case SerializationTag::Multisignature: {
    VarNote::MultisignatureInputDetails v;
    serializer(v, "data");
    in = v;
    break;
  }

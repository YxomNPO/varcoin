...
namespace VarNote
{
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  core_rpc_server::core_rpc_server(core& cr, nodetool::node_server<VarNote::t_VarNote_protocol_handler<VarNote::core> >& p2p):m_core(cr), m_p2p(p2p)
  {}
  //------------------------------------------------------------------------------------------------------------------------------
...
#define CHECK_CORE_READY() if(!check_core_ready()){res.status =  CORE_RPC_STATUS_BUSY;return true;}
  ...
  bool core_rpc_server::on_send_raw_tx(const COMMAND_RPC_SEND_RAW_TX::request& req, COMMAND_RPC_SEND_RAW_TX::response& res, connection_context& cntx)
  {
    ...
    VarNote_connection_context fake_context = AUTO_VAL_INIT(fake_context);
  ...
  bool core_rpc_server::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res, epee::json_rpc::error& error_resp, connection_context& cntx)
  {
    ...
    VarNote::account_public_address acc = AUTO_VAL_INIT(acc);

    if(!req.wallet_address.size() || !VarNote::get_account_address_from_str(acc, req.wallet_address))
    {
    ...
    crypto::public_key tx_pub_key = VarNote::get_tx_pub_key_from_extra(b.miner_tx);

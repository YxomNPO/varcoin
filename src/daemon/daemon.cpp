...
#include "VarNoteCore/MinerConfig.h"
...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file", std::string(VarNote::VARNOTE_NAME) + ".conf"};
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      {
        std::cout << VarNote::VARNOTE_NAME << " v" << PROJECT_VERSION_LONG << ENDL << ENDL;
    ...
    logger(INFO) << VarNote::VARNOTE_NAME << " v" << PROJECT_VERSION_LONG;
    ...
    VarNote::CurrencyBuilder currencyBuilder(logManager);
    currencyBuilder.testnet(testnet_mode);
    VarNote::Currency currency = currencyBuilder.currency();
    VarNote::core ccore(currency, nullptr, logManager);

    VarNote::Checkpoints checkpoints(logManager);
    for (const auto& cp : VarNote::CHECKPOINTS) {
    ...
    VarNote::VarNoteProtocolHandler cprotocol(currency, dispatcher, ccore, nullptr, logManager);
    VarNote::NodeServer p2psrv(dispatcher, cprotocol, logManager);
    VarNote::RpcServer rpcServer(dispatcher, logManager, ccore, p2psrv);
...
bool command_line_preprocessor(const boost::program_options::variables_map &vm, LoggerRef &logger) {
  ...
  if (command_line::get_arg(vm, command_line::arg_version)) {
    std::cout << VarNote::VARNOTE_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    exit = true;
  }

...
{
  nodetool::node_server<VarNote::t_VarNote_protocol_handler<VarNote::core> >& m_srv;
public:
  daemon_cmmands_handler(nodetool::node_server<VarNote::t_VarNote_protocol_handler<VarNote::core> >& srv):m_srv(srv)
  {
...
private:
  epee::srv_console_handlers_binder<nodetool::node_server<VarNote::t_VarNote_protocol_handler<VarNote::core> > > m_cmd_binder;

  //--------------------------------------------------------------------------------
  std::string get_commands_str()
  {
    std::stringstream ss;
    ss << VarNote_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    ss << "Commands: " << ENDL;
  ...
  {
    std::cout << VarNote::obj_to_json_str(obj) << ENDL;
    return true;
  }
  //--------------------------------------------------------------------------------
  bool print_block_by_height(uint64_t height)
  {
    ...
    if (1 == blocks.size())
    {
      VarNote::block& block = blocks.front();
  ...
  bool print_tx(const std::vector<std::string>& args)
  {
    ...
    if (1 == txs.size())
    {
      VarNote::transaction tx = txs.front();
      print_as_json(tx);
    }
    else
    {
  ...
  bool start_mining(const std::vector<std::string>& args)
  {
    ...
    VarNote::account_public_address adr;
    if(!VarNote::get_account_address_from_str(adr, args.front()))
    {

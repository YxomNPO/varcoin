...
WalletLegacySerializer::WalletLegacySerializer(VarNote::AccountBase& account, WalletUserTransactionsCache& transactionsCache) :
  account(account),
...
void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& cache) {
  ...
  VarNote::BinaryOutputStreamSerializer serializer(plainStream);
  saveKeys(serializer);
...
void WalletLegacySerializer::saveKeys(VarNote::ISerializer& serializer) {
  VarNote::KeysStorage keys;
  VarNote::AccountKeys acc = account.getAccountKeys();
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& cache) {
  ...
  VarNote::BinaryInputStreamSerializer serializerEncrypted(stdStream);
...
void WalletLegacySerializer::loadKeys(VarNote::ISerializer& serializer) {
  VarNote::KeysStorage keys;

  keys.serialize(serializer, "keys");

  VarNote::AccountKeys acc;

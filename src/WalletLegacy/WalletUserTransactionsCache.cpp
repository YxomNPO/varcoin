...
bool WalletUserTransactionsCache::serialize(VarNote::ISerializer& s) {
...
void WalletUserTransactionsCache::updateTransaction(
  TransactionId transactionId, const VarNote::Transaction& tx, uint64_t amount, const std::list<TransactionOutputInformation>& usedOutputs) {
...
std::shared_ptr<WalletLegacyEvent> WalletUserTransactionsCache::onTransactionUpdated(const TransactionInformation& txInfo, int64_t txBalance) {
  ...
  TransactionId id = VarNote::WALLET_LEGACY_INVALID_TRANSACTION_ID;
  ...
  if (id == VarNote::WALLET_LEGACY_INVALID_TRANSACTION_ID) {
    WalletLegacyTransaction transaction;
...
TransactionId WalletUserTransactionsCache::findTransactionByHash(const Hash& hash) {
  ...
  if (it == m_transactions.end())
    return VarNote::WALLET_LEGACY_INVALID_TRANSACTION_ID;

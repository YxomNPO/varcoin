...
{
public:
  ContextCounterHolder(VarNote::WalletAsyncContextCounter& shutdowner) : m_shutdowner(shutdowner) {}
  ~ContextCounterHolder() { m_shutdowner.delAsyncContext(); }

private:
  VarNote::WalletAsyncContextCounter& m_shutdowner;
};
...
class InitWaiter : public VarNote::IWalletLegacyObserver {
public:
...
class SaveWaiter : public VarNote::IWalletLegacyObserver {
public:
...
class SyncStarter : public VarNote::IWalletLegacyObserver {
public:
...
WalletLegacy::WalletLegacy(const VarNote::Currency& currency, INode& node) :
...
void WalletLegacy::doLoad(std::istream& source) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = WalletLegacy::NOT_INITIALIZED;} );
    m_observerManager.notify(&IWalletLegacyObserver::initCompleted, make_error_code(VarNote::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
void WalletLegacy::save(std::ostream& destination, bool saveDetailed, bool saveCache) {
  if(m_isStopping) {
    m_observerManager.notify(&IWalletLegacyObserver::saveCompleted, make_error_code(VarNote::error::OPERATION_CANCELLED));
    return;
  }

  {
    ...
    throwIf(m_state != INITIALIZED, VarNote::error::WRONG_STATE);
...
void WalletLegacy::doSave(std::ostream& destination, bool saveDetailed, bool saveCache) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = WalletLegacy::INITIALIZED;} );
    m_observerManager.notify(&IWalletLegacyObserver::saveCompleted, make_error_code(VarNote::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
std::error_code WalletLegacy::changePassword(const std::string& oldPassword, const std::string& newPassword) {
  ...
  if (m_password.compare(oldPassword))
    return make_error_code(VarNote::error::WRONG_PASSWORD);
...
std::error_code WalletLegacy::cancelTransaction(size_t transactionId) {
  return make_error_code(VarNote::error::TX_CANCEL_IMPOSSIBLE);
}

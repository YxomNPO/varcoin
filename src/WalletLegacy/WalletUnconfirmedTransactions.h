...
template<> 
struct hash<VarNote::TransactionOutputId> {
  size_t operator()(const VarNote::TransactionOutputId &_v) const {    
...
public:
  ...
  bool serialize(VarNote::ISerializer& s);

  bool findTransactionId(const Crypto::Hash& hash, TransactionId& id);
  void erase(const Crypto::Hash& hash);
  void add(const VarNote::Transaction& tx, TransactionId transactionId, 

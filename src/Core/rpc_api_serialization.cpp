...
void ser_members(varcoin::SendProof &v, ISeria &s) {
	const uint64_t VARNOTE_PUBLIC_ADDRESS_BASE58_PREFIX = 6;  // TODO - hard to get reference to our currency here
	std::string addr;
	if (!s.is_input())
		addr = Currency::get_account_address_as_str(VARNOTE_PUBLIC_ADDRESS_BASE58_PREFIX, v.address);
	seria_kv("address", addr, s);
	uint64_t prefix = 0;
	if (s.is_input() && (!Currency::parse_account_address_string(&prefix, &v.address, addr) ||
	                        prefix != VARNOTE_PUBLIC_ADDRESS_BASE58_PREFIX))
...
void ser_members(api::varcoind::GetStatus::Request &v, ISeria &s) {
...
void ser_members(api::varcoind::GetStatus::Response &v, ISeria &s) {
	ser_members(static_cast<api::varcoind::GetStatus::Request &>(v), s);
...
void ser_members(api::varcoind::SyncBlocks::Request &v, ISeria &s) {
...
void ser_members(api::varcoind::SyncBlocks::Response &v, ISeria &s) {
	seria_kv("blocks", v.blocks, s);
	seria_kv("start_height", v.start_height, s);
	seria_kv("status", v.status, s);
}
void ser_members(api::varcoind::GetRawTransaction::Request &v, ISeria &s) { seria_kv("hash", v.hash, s); }
void ser_members(api::varcoind::GetRawTransaction::Response &v, ISeria &s) {
	seria_kv("transaction", v.transaction, s);
	seria_kv("raw_transaction", v.raw_transaction, s);
}

void ser_members(api::varcoind::SyncMemPool::Request &v, ISeria &s) {
	if (!s.is_input())
...
void ser_members(api::varcoind::SyncMemPool::Response &v, ISeria &s) {
...
void ser_members(api::varcoind::GetRandomOutputs::Request &v, ISeria &s) {
	seria_kv("amounts", v.amounts, s);
	seria_kv("outs_count", v.outs_count, s);
	seria_kv("confirmed_height_or_depth", v.confirmed_height_or_depth, s);
}
void ser_members(api::varcoind::GetRandomOutputs::Response &v, ISeria &s) { seria_kv("outputs", v.outputs, s); }
void ser_members(api::varcoind::SendTransaction::Request &v, ISeria &s) {
	seria_kv("binary_transaction", v.binary_transaction, s);
}
void ser_members(api::varcoind::SendTransaction::Response &v, ISeria &s) { seria_kv("send_result", v.send_result, s); }
...
//}
/*void ser_members(varcoin::api::walletd::GetBlock::Request &v, ISeria &s) {
        seria_kv("hash", v.hash, s);
        seria_kv("height", v.height, s);
}*/
void ser_members(varcoin::api::walletd::GetTransaction::Request &v, ISeria &s) { seria_kv("hash", v.hash, s); }
...
void ser_members(varcoin::api::varcoind::SubmitBlock::Response &v, ISeria &s) { seria_kv("status", v.status, s); }
...
void ser_members(varcoin::api::varcoind::BlockHeaderLegacy &v, ISeria &s) {
	ser_members(static_cast<varcoin::api::BlockHeader &>(v), s);

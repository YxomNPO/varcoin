...
/*struct WalletTransactionDto {
        ...
        WalletTransactionDto(const varcoin::WalletTransaction &wallet) {
                state         = wallet.state;
        ...
        varcoin::WalletTransactionState state;
        uint64_t timestamp;
        uint32_t block_height;
        Hash hash;
...
version(version) {}
        WalletTransferDto(const varcoin::WalletTransfer &tr, uint32_t version)
...
/*void serialize(WalletRecordDto &v, varcoin::ISerializer &s) {
...
std::string read_cipher(common::IInputStream &source, const std::string &name) {
	std::string cipher;
	//	varcoin::BinaryInputStreamSerializer s(source);
...
std::string decrypt(const std::string &cipher, varcoin::WalletSerializerV1::CryptoContext &crypto_ctx) {
	std::string plain;
...
void deserialize_encrypted(Object &obj, const std::string &name,
    varcoin::WalletSerializerV1::CryptoContext &crypto_ctx, common::IInputStream &source) {

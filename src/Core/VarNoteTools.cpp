...
Hash varcoin::get_base_transaction_hash(const BaseTransaction &tx) {
	if (tx.version < 2)
...
void varcoin::decompose_amount(Amount amount, Amount dust_threshold, std::vector<Amount> *decomposed_amounts) {
...
size_t varcoin::get_maximum_tx_size(size_t input_count, size_t output_count, size_t mixin_count) {
...
bool varcoin::get_tx_fee(const Transaction &tx, uint64_t *fee) {
...
uint64_t varcoin::get_tx_fee(const Transaction &tx) {
	uint64_t r = 0;
	if (!get_tx_fee(tx, &r))
		return 0;
	return r;

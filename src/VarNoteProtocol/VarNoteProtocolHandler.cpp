...
#include "VarNoteProtocolHandler.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const VarNoteConnectionContext& context) {
...
VarNoteProtocolHandler::VarNoteProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t VarNoteProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void VarNoteProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void VarNoteProtocolHandler::onConnectionOpened(VarNoteConnectionContext& context) {
}

void VarNoteProtocolHandler::onConnectionClosed(VarNoteConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IVarNoteProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != VarNoteConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IVarNoteProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void VarNoteProtocolHandler::stop() {
  m_stop = true;
}

bool VarNoteProtocolHandler::start_sync(VarNoteConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == VarNoteConnectionContext::state_synchronizing) {
...
bool VarNoteProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void VarNoteProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const VarNoteConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t VarNoteProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool VarNoteProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, VarNoteConnectionContext& context, bool is_inital) {
  if (context.m_state == VarNoteConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == VarNoteConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = VarNoteConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = VarNoteConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = VarNoteConnectionContext::state_sync_required;
  }
...
bool VarNoteProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, VarNoteConnectionContext& ctx, Handler handler) {
...
int VarNoteProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, VarNoteConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &VarNoteProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &VarNoteProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &VarNoteProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &VarNoteProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &VarNoteProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &VarNoteProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &VarNoteProtocolHandler::handleRequestTxPool)
...
int VarNoteProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, VarNoteConnectionContext& context) {
  ...
  if (context.m_state != VarNoteConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    VarNote::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = VarNoteConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = VarNoteConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int VarNoteProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, VarNoteConnectionContext& context) {
...
int VarNoteProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, VarNoteConnectionContext& context) {
...
int VarNoteProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, VarNoteConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = VarNoteConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == VarNoteConnectionContext::state_synchronizing) {
...
int VarNoteProtocolHandler::processObjects(VarNoteConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool VarNoteProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int VarNoteProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, VarNoteConnectionContext& context) {
...
bool VarNoteProtocolHandler::request_missing_objects(VarNoteConnectionContext& context, bool check_having_blocks) {
...
bool VarNoteProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IVarNoteProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int VarNoteProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, VarNoteConnectionContext& context) {
...
int VarNoteProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     VarNoteConnectionContext& context) {
...
void VarNoteProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void VarNoteProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void VarNoteProtocolHandler::requestMissingPoolTransactions(const VarNoteConnectionContext& context) {
...
void VarNoteProtocolHandler::updateObservedHeight(uint32_t peerHeight, const VarNoteConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void VarNoteProtocolHandler::recalculateMaxObservedHeight(const VarNoteConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const VarNoteConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t VarNoteProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool VarNoteProtocolHandler::addObserver(IVarNoteProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool VarNoteProtocolHandler::removeObserver(IVarNoteProtocolObserver* observer) {

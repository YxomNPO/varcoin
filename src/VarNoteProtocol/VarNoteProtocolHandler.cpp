...
CoreStatistics VarNoteProtocolHandler::getStatistics() {
...
#define HANDLE_NOTIFY(CMD, Handler) case CMD::ID: { ret = notifyAdaptor<CMD>(in, ctx, [this](int a1, CMD::request& a2, VarNoteConnectionContext& a3) { return Handler(a1, a2, a3); }); break; }
...
int VarNoteProtocolHandler::processObjects(VarNoteConnectionContext& context, std::vector<RawBlock>&& rawBlocks, const std::vector<CachedBlock>& cachedBlocks) {
...
bool VarNoteProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IVarNoteProtocolObserver::blockchainSynchronized, m_core.getTopBlockIndex());
  }
  return true;
}
...
void VarNoteProtocolHandler::relayBlock(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void VarNoteProtocolHandler::relayTransactions(const std::vector<BinaryArray>& transactions) {

...
Crypto::Hash BlockchainMonitor::requestLastBlockHash() {
  ...
  try {
    VarNote::HttpClient client(m_dispatcher, m_daemonHost, m_daemonPort);

    VarNote::COMMAND_RPC_GET_LAST_BLOCK_HEADER::request request;
    VarNote::COMMAND_RPC_GET_LAST_BLOCK_HEADER::response response;

    System::EventLock lk(m_httpEvent);
    VarNote::JsonRpc::invokeJsonRpcCommand(client, "getlastblockheader", request, response);

...
class InProcessNode : public INode, public VarNote::IVarNoteProtocolObserver {
public:
  //NOTE: dispatcher must be the same as the one used in core and protocol
  InProcessNode(VarNote::ICore& core, VarNote::IVarNoteProtocolHandler& protocol, System::Dispatcher& dispatcher);
...
private:
  ...
  std::error_code doGetNewBlocks(const std::vector<Crypto::Hash>& knownBlockIds, std::vector<VarNote::RawBlock>& newBlocks, uint32_t& startHeight);
  ...
  VarNote::IVarNoteProtocolHandler& protocol;

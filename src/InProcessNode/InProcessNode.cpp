...
InProcessNode::InProcessNode(VarNote::ICore& core, VarNote::IVarNoteProtocolHandler& protocol,
...
void InProcessNode::getNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds,
                                 std::vector<VarNote::RawBlock>& newBlocks, uint32_t& startIndex,
...
std::error_code InProcessNode::doGetNewBlocks(const std::vector<Crypto::Hash>& knownBlockIds,
                                              std::vector<VarNote::RawBlock>& newBlocks, uint32_t& startHeight) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
  ...
  try {
    ...
    if (knownBlockIds.back() != VarNote::CachedBlock(blockTemplate).getBlockHash()) {
      return make_error_code(VarNote::error::REQUEST_ERROR);
    }

    uint32_t totalBlockCount;
    auto supplement = core.findBlockchainSupplement(knownBlockIds, VarNote::COMMAND_RPC_GET_BLOCKS_FAST_MAX_COUNT,
...
void InProcessNode::getRandomOutsByAmounts(
    ...
    std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result,
    const Callback& callback) {
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
...
std::error_code InProcessNode::doGetRandomOutsByAmounts(
  ...
  try {
    ...
    std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> tmpResult;
    for (auto amount : amounts) {
      ...
      VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount outsForAmount;
...
void InProcessNode::resetLastLocalBlockHeaderInfo() {
  ...
  lastLocalBlockHeaderInfo.hash = VarNote::NULL_HASH;
  lastLocalBlockHeaderInfo.prevHash = VarNote::NULL_HASH;

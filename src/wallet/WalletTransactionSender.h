...
public:
  ...
  void init(VarNote::account_keys keys);
  void stop();
...
private:
  ...
  void prepareInputs(const std::list<size_t>& selectedTransfers, std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& outs,
      std::vector<VarNote::tx_source_entry>& sources, uint64_t mixIn);
  void splitDestinations(TransferId firstTransferId, size_t transfersCount, const VarNote::tx_destination_entry& changeDts,
      const TxDustPolicy& dustPolicy, std::vector<VarNote::tx_destination_entry>& splittedDests);
  void digitSplitStrategy(TransferId firstTransferId, size_t transfersCount, const VarNote::tx_destination_entry& change_dst, uint64_t dust_threshold,

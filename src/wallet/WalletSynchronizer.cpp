...
void throwIf(bool expr, VarNote::error::WalletErrorCodes ec) {
  if (expr)
    throw std::system_error(make_error_code(ec));
}

bool getTxPubKey(const VarNote::transaction& tx, crypto::public_key& key) {
  std::vector<VarNote::tx_extra_field> extraFields;
  VarNote::parse_tx_extra(tx.extra, extraFields);

  VarNote::tx_extra_pub_key pubKeyField;
  if(!VarNote::find_tx_extra_field_by_type(extraFields, pubKeyField)) {
...
void findMyOuts(const VarNote::account_keys& acc, const VarNote::transaction& tx, const crypto::public_key& txPubKey, std::vector<size_t>& outs, uint64_t& moneyTransfered) {
  bool r = VarNote::lookup_acc_outs(acc, tx, txPubKey, outs, moneyTransfered);
  throwIf(!r, VarNote::error::INTERNAL_WALLET_ERROR);
}

uint64_t countOverallTxOutputs(const VarNote::transaction& tx) {
  uint64_t amount = 0;
  for (const VarNote::tx_out& o: tx.vout) {
    amount += o.amount;
  }
...
uint64_t countOverallTxInputs(const VarNote::transaction& tx) {
  uint64_t amount = 0;
  for (auto& in: tx.vin) {
    if(in.type() != typeid(VarNote::txin_to_key))
      continue;

    amount += boost::get<VarNote::txin_to_key>(in).amount;
  }
...
WalletSynchronizer::WalletSynchronizer(const VarNote::account_base& account, INode& node, std::vector<crypto::hash>& blockchain,
...
void WalletSynchronizer::handleNewBlocksPortion(std::shared_ptr<SynchronizationContext> context, std::deque<std::shared_ptr<WalletEvent> >& events,
  ...
  try
  {
    ...
    parameters.nextRequest = boost::none;
    parameters.events.push_back(std::make_shared<WalletSynchronizationProgressUpdatedEvent>(context->startHeight, m_node.getLastLocalBlockHeight(), make_error_code(VarNote::error::INTERNAL_WALLET_ERROR)));
  }
...
WalletSynchronizer::NextBlockAction WalletSynchronizer::handleNewBlockchainEntry(ProcessParameters& parameters, VarNote::block_complete_entry& blockEntry, uint64_t height) {
  VarNote::block b;
  bool r = VarNote::parse_and_validate_block_from_blob(blockEntry.block, b);
  throwIf(!r, VarNote::error::INTERNAL_WALLET_ERROR);
  ...
  if(blockId != m_blockchain[height]) {
    ...
    throwIf(height == parameters.context->startHeight, VarNote::error::INTERNAL_WALLET_ERROR);
...
bool WalletSynchronizer::processNewBlockchainEntry(ProcessParameters& parameters, VarNote::block_complete_entry& blockEntry, const VarNote::block& b, crypto::hash& blockId, uint64_t height) {
  throwIf(height != m_blockchain.size(), VarNote::error::INTERNAL_WALLET_ERROR);
...
bool WalletSynchronizer::processMinersTx(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t height, uint64_t timestamp) {
  bool r = true;
...
bool WalletSynchronizer::processNewTransaction(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t height, bool isCoinbase, uint64_t timestamp) {
  bool res = true;
...
uint64_t WalletSynchronizer::processMyInputs(const VarNote::transaction& tx) {
  uint64_t money = 0;
  ...
  for (auto& in: tx.vin) {
    ...
    size_t idx;
    if (!m_transferDetails.getTransferDetailsIdxByKeyImage(boost::get<VarNote::txin_to_key>(in).k_image, idx))
      continue;

    money += boost::get<VarNote::txin_to_key>(in).amount;
...
void WalletSynchronizer::fillGetTransactionOutsGlobalIndicesRequest(ProcessParameters& parameters, const VarNote::transaction& tx,
    const std::vector<size_t>& outs, const crypto::public_key& publicKey, uint64_t height) {
  crypto::hash txid = VarNote::get_transaction_hash(tx);
...
void WalletSynchronizer::updateTransactionsCache(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t myOuts, uint64_t myInputs, uint64_t height, bool isCoinbase, uint64_t timestamp) {

  uint64_t allOuts = countOverallTxOutputs(tx);
  uint64_t allInputs = countOverallTxInputs(tx);

  TransactionId foundTx = m_transactionsCache.findTransactionByHash(VarNote::get_transaction_hash(tx));
...
void WalletSynchronizer::processUnconfirmed(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t height, uint64_t timestamp) {
  TransactionId id;
...
void WalletSynchronizer::handleTransactionOutGlobalIndicesResponse(std::shared_ptr<SynchronizationContext> context, crypto::hash txid, uint64_t height,
  ...
  try
  {
    ...
    if (it == context->transactionContext.end()) {
      events.push_back(std::make_shared<WalletSynchronizationProgressUpdatedEvent>(height, m_node.getLastLocalBlockHeight(), make_error_code(VarNote::error::INTERNAL_WALLET_ERROR)));
      return;
    }

    VarNote::transaction& tx = it->second.transaction;
    ...
    for (size_t o: outs) {
      throwIf(tx.vout.size() <= o, VarNote::error::INTERNAL_WALLET_ERROR);
      ...
      VarNote::generate_key_image_helper(m_account.get_keys(), tx_pub_key, o, in_ephemeral, td.keyImage);
      throwIf(in_ephemeral.pub != boost::get<VarNote::txout_to_key>(tx.vout[o].target).key, VarNote::error::INTERNAL_WALLET_ERROR);

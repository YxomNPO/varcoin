...
namespace tools
{
  ...
  {
    std::vector<VarNote::tx_destination_entry> dsts;
    ...
    if (!req.payment_id.empty()) {
      ...
      VarNote::set_payment_id_to_tx_extra_nonce(extra_nonce, payment_id);
      if (!VarNote::add_extra_nonce_to_tx_extra(extra, extra_nonce)) {
        er.code = WALLET_RPC_ERROR_CODE_WRONG_PAYMENT_ID;
    ...
    try {
      ...
      res.tx_hash = boost::lexical_cast<std::string>(VarNote::get_transaction_hash(tx));
      return true;
  ...
  {
    crypto::hash payment_id;
    VarNote::blobdata payment_id_blob;

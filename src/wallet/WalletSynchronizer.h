...
{
public:
  WalletSynchronizer(const VarNote::account_base& account, INode& node, std::vector<crypto::hash>& blockchain, WalletTransferDetails& transferDetails,
...
private:
  ...
  NextBlockAction handleNewBlockchainEntry(ProcessParameters& parameters, VarNote::block_complete_entry& blockEntry, uint64_t height);
  bool processNewBlockchainEntry(ProcessParameters& parameters, VarNote::block_complete_entry& blockEntry, const VarNote::block& b, crypto::hash& blockId, uint64_t height);
  bool processNewTransaction(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t height, bool isCoinbase, uint64_t timestamp);
  bool processMinersTx(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t height, uint64_t timestamp);
  void processUnconfirmed(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t height, uint64_t timestamp);
  uint64_t processMyInputs(const VarNote::transaction& tx);
  void updateTransactionsCache(ProcessParameters& parameters, const VarNote::transaction& tx, uint64_t myOuts, uint64_t myInputs, uint64_t height,
  ...
  void fillGetTransactionOutsGlobalIndicesRequest(ProcessParameters& parameters, const VarNote::transaction& tx,
  ...
  const VarNote::account_base& m_account;
  INode& m_node;

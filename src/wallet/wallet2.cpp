...
#include "VarNote_protocol/blobdatatype.h"
...
//----------------------------------------------------------------------------------------------------
void wallet2::process_new_transaction(const VarNote::transaction& tx, uint64_t height)
{
  ...
  if(!outs.empty() && tx_money_got_in_outs)
  {
    ...
    VarNote::COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES::request req = AUTO_VAL_INIT(req);
    VarNote::COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES::response res = AUTO_VAL_INIT(res);
    ...
    BOOST_FOREACH(size_t o, outs)
    {
      ...
      VarNote::generate_key_image_helper(m_account.get_keys(), tx_pub_key, o, in_ephemeral, td.m_key_image);
      THROW_WALLET_EXCEPTION_IF(in_ephemeral.pub != boost::get<VarNote::txout_to_key>(tx.vout[o].target).key,
  ...
  BOOST_FOREACH(auto& in, tx.vin)
  {
    if(in.type() != typeid(VarNote::txin_to_key))
      continue;
    auto it = m_key_images.find(boost::get<VarNote::txin_to_key>(in).k_image);
    if(it != m_key_images.end())
    {
      LOG_PRINT_L0("Spent money: " << print_money(boost::get<VarNote::txin_to_key>(in).amount) << ", with tx: " << get_transaction_hash(tx));
      tx_money_spent_in_ins += boost::get<VarNote::txin_to_key>(in).amount;
  ...
  if (find_tx_extra_field_by_type(tx_extra_fields, extra_nonce))
  {
    ...
    if(get_payment_id_from_tx_extra_nonce(extra_nonce.nonce, payment_id))
    {
      ...
      if (0 < received && null_hash != payment_id)
      {
        ...
        payment.m_tx_hash      = VarNote::get_transaction_hash(tx);
...
//----------------------------------------------------------------------------------------------------
void wallet2::process_unconfirmed(const VarNote::transaction& tx)
{
...
//----------------------------------------------------------------------------------------------------
void wallet2::process_new_blockchain_entry(const VarNote::block& b, VarNote::block_complete_entry& bche, crypto::hash& bl_id, uint64_t height)
{
...
{
  blocks_added = 0;
  VarNote::COMMAND_RPC_GET_BLOCKS_FAST::request req = AUTO_VAL_INIT(req);
  VarNote::COMMAND_RPC_GET_BLOCKS_FAST::response res = AUTO_VAL_INIT(res);
  ...
  {
    VarNote::block bl;
    r = VarNote::parse_and_validate_block_from_blob(bl_entry.block, bl);
...
void wallet2::load_keys(const std::string& keys_file_name, const std::string& password)
{
  ...
  const VarNote::account_keys& keys = m_account.get_keys();
...
bool wallet2::parse_payment_id(const std::string& payment_id_str, crypto::hash& payment_id) {
  VarNote::blobdata payment_id_data;
...
{
  if(unlock_time < VarNote_MAX_BLOCK_NUMBER)
  {
    //interpret as block index
    if(m_blockchain.size()-1 + VarNote_LOCKED_TX_ALLOWED_DELTA_BLOCKS >= unlock_time)
      return true;
    else
      return false;
  }else
  {
    ...
    if(current_time + VarNote_LOCKED_TX_ALLOWED_DELTA_SECONDS >= unlock_time)
      return true;
    else
      return false;
  }
...
//----------------------------------------------------------------------------------------------------
void wallet2::add_unconfirmed_tx(const VarNote::transaction& tx, uint64_t change_amount)
{
  unconfirmed_transfer_details& utd = m_unconfirmed_txs[VarNote::get_transaction_hash(tx)];
...
void wallet2::transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count,
                       uint64_t unlock_time, uint64_t fee, const std::vector<uint8_t>& extra, VarNote::transaction& tx)
{
...
//----------------------------------------------------------------------------------------------------
void wallet2::generateGenesis(VarNote::block& b) {
  if (m_testnet) {
    VarNote::generateTestnetGenesisBlock(b);
  } else {
    VarNote::generateGenesisBlock(b);
  }
}
//----------------------------------------------------------------------------------------------------
}

...
{
  if(unlockTime < VarNote_MAX_BLOCK_NUMBER) {
    //interpret as block index
    if(m_blockchain.size()-1 + VarNote_LOCKED_TX_ALLOWED_DELTA_BLOCKS >= unlockTime)
      return true;
    else
      return false;
  }
  else
  {
    ...
    if(current_time + VarNote_LOCKED_TX_ALLOWED_DELTA_SECONDS >= unlockTime)
      return true;
    else
      return false;
  }
...
void WalletTransferDetails::detachTransferDetails(size_t height) {
  ...
  for(size_t i = start; i!= m_transfers.size();i++) {
    ...
    if(ki == m_keyImages.end()) throw std::system_error(make_error_code(VarNote::error::INTERNAL_WALLET_ERROR));

...
namespace tools
{
  ...
  struct tx_dust_policy
  {
    ...
    bool add_to_fee;
    VarNote::account_public_address addr_for_dust;

    tx_dust_policy(uint64_t a_dust_threshold = 0, bool an_add_to_fee = true, VarNote::account_public_address an_addr_for_dust = VarNote::account_public_address())
  ...
  public:
    ...
    void store();
    VarNote::account_base& get_account(){return m_account;}

    void init(const std::string& daemon_address = "http://localhost:8080", uint64_t upper_transaction_size_limit = VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE*2 - VarNote_COINBASE_BLOB_RESERVED_SIZE);
    bool deinit();
    ...
    template<typename T>
    void transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count, uint64_t unlock_time, uint64_t fee, const std::vector<uint8_t>& extra, T destination_split_strategy, const tx_dust_policy& dust_policy);
    template<typename T>
    void transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count, uint64_t unlock_time, uint64_t fee, const std::vector<uint8_t>& extra, T destination_split_strategy, const tx_dust_policy& dust_policy, VarNote::transaction &tx);
    void transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count, uint64_t unlock_time, uint64_t fee, const std::vector<uint8_t>& extra);
    void transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count, uint64_t unlock_time, uint64_t fee, const std::vector<uint8_t>& extra, VarNote::transaction& tx);
  ...
  private:
    ...
    void process_new_transaction(const VarNote::transaction& tx, uint64_t height);
    void process_new_blockchain_entry(const VarNote::block& b, VarNote::block_complete_entry& bche, crypto::hash& bl_id, uint64_t height);
    ...
    void process_unconfirmed(const VarNote::transaction& tx);
    void add_unconfirmed_tx(const VarNote::transaction& tx, uint64_t change_amount);
    void generateGenesis(VarNote::block& b);
    void checkGenesis(const crypto::hash& genesis_hash); //throws

    VarNote::account_base m_account;
    ...
    VarNote::account_public_address m_account_public_address;
...
namespace tools
{
  ...
  {
    //----------------------------------------------------------------------------------------------------
    inline void digit_split_strategy(const std::vector<VarNote::tx_destination_entry>& dsts,
      ...
      std::vector<VarNote::tx_destination_entry>& splitted_dsts, uint64_t& dust)
    {
      ...
      BOOST_FOREACH(auto& de, dsts)
      {
        VarNote::decompose_amount_into_digits(de.amount, dust_threshold,
          [&](uint64_t chunk) { splitted_dsts.push_back(VarNote::tx_destination_entry(chunk, de.addr)); },
          [&](uint64_t a_dust) { splitted_dsts.push_back(VarNote::tx_destination_entry(a_dust, de.addr)); } );
      }
    ...
    //----------------------------------------------------------------------------------------------------
    inline void null_split_strategy(const std::vector<VarNote::tx_destination_entry>& dsts,
      const VarNote::tx_destination_entry& change_dst, uint64_t dust_threshold,
      std::vector<VarNote::tx_destination_entry>& splitted_dsts, uint64_t& dust)
    {
      splitted_dsts = dsts;
      ...
      if (0 != change)
      {
        splitted_dsts.push_back(VarNote::tx_destination_entry(change, change_dst.addr));
      }
    }
    //----------------------------------------------------------------------------------------------------
    inline void print_source_entry(const VarNote::tx_source_entry& src)
    {
      std::string indexes;
      std::for_each(src.outputs.begin(), src.outputs.end(), [&](const VarNote::tx_source_entry::output_entry& s_e) { indexes += boost::to_string(s_e.first) + " "; });
      LOG_PRINT_L0("amount=" << VarNote::print_money(src.amount) << ", real_output=" <<src.real_output << ", real_output_in_tx_index=" << src.real_output_in_tx_index << ", indexes: " << indexes);
    }
    //----------------------------------------------------------------------------------------------------
  }
  //----------------------------------------------------------------------------------------------------
  template<typename T>
  void wallet2::transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count,
  ...
  template<typename T>
  void wallet2::transfer(const std::vector<VarNote::tx_destination_entry>& dsts, size_t fake_outputs_count,
    uint64_t unlock_time, uint64_t fee, const std::vector<uint8_t>& extra, T destination_split_strategy, const tx_dust_policy& dust_policy, VarNote::transaction &tx)
  {
    ...
    typedef VarNote::tx_source_entry::output_entry tx_output_entry;
    ...
    VarNote::tx_destination_entry change_dts = AUTO_VAL_INIT(change_dts);
    ...
    uint64_t dust = 0;
    std::vector<VarNote::tx_destination_entry> splitted_dsts;
    ...
    {
      splitted_dsts.push_back(VarNote::tx_destination_entry(dust, dust_policy.addr_for_dust));
    }

    bool r = VarNote::construct_tx(m_account.get_keys(), sources, splitted_dsts, extra, tx, unlock_time);

...
void Wallet::doLoad(std::istream& source) {
  try
  {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = Wallet::NOT_INITIALIZED;} );
    m_observerManager.notify(&IWalletObserver::initCompleted, make_error_code(VarNote::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
void Wallet::save(std::ostream& destination, bool saveDetailed, bool saveCache) {
  if(m_isStopping) {
    m_observerManager.notify(&IWalletObserver::saveCompleted, make_error_code(VarNote::error::OPERATION_CANCELLED));
    return;
  }
...
void Wallet::doSave(std::ostream& destination, bool saveDetailed, bool saveCache) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = Wallet::INITIALIZED;} );
    m_observerManager.notify(&IWalletObserver::saveCompleted, make_error_code(VarNote::error::INTERNAL_WALLET_ERROR));
    return;
  }

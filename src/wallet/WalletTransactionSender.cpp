...
uint64_t countNeededMoney(uint64_t fee, const std::vector<VarNote::Transfer>& transfers) {
  uint64_t needed_money = fee;
  for (auto& transfer: transfers) {
    VarNote::throwIf(transfer.amount == 0, VarNote::error::ZERO_DESTINATION);
    VarNote::throwIf(transfer.amount < 0, VarNote::error::WRONG_AMOUNT);

    needed_money += transfer.amount;
    VarNote::throwIf(static_cast<int64_t>(needed_money) < transfer.amount, VarNote::error::SUM_OVERFLOW);
  }

  return needed_money;
}

void createChangeDestinations(const VarNote::account_public_address& address, uint64_t neededMoney, uint64_t foundMoney, VarNote::tx_destination_entry& changeDts) {
...
void constructTx(const VarNote::account_keys keys, const std::vector<VarNote::tx_source_entry>& sources, const std::vector<VarNote::tx_destination_entry>& splittedDests,
    const std::string& extra, uint64_t unlockTimestamp, uint64_t sizeLimit, VarNote::transaction& tx) {
  std::vector<uint8_t> extraVec;
  extraVec.reserve(extra.size());
  std::for_each(extra.begin(), extra.end(), [&extraVec] (const char el) { extraVec.push_back(el);});

  bool r = VarNote::construct_tx(keys, sources, splittedDests, extraVec, tx, unlockTimestamp);
  VarNote::throwIf(!r, VarNote::error::INTERNAL_WALLET_ERROR);
  VarNote::throwIf(VarNote::get_object_blobsize(tx) >= sizeLimit, VarNote::error::TRANSACTION_SIZE_TOO_BIG);
}

void fillTransactionHash(const VarNote::transaction& tx, VarNote::TransactionHash& hash) {
  crypto::hash h = VarNote::get_transaction_hash(tx);
...
WalletTransactionSender::WalletTransactionSender(WalletUserTransactionsCache& transactionsCache, WalletTxSendingState& sendingTxsStates,
    WalletTransferDetails& transferDetails, WalletUnconfirmedTransactions& unconfirmedTransactions):
      ...
      m_isStoping(false) {
  m_upperTransactionSizeLimit = VarNote_BLOCK_GRANTED_FULL_REWARD_ZONE*2 - VarNote_COINBASE_BLOB_RESERVED_SIZE;
}

void WalletTransactionSender::init(VarNote::account_keys keys) {
  if (m_isInitialized) {
...
std::shared_ptr<WalletRequest> WalletTransactionSender::makeSendRequest(TransactionId& transactionId, std::deque<std::shared_ptr<WalletEvent> >& events,
  ...
  throwIf(transfers.empty(), VarNote::error::ZERO_DESTINATION);
  ...
  throwIf(context->foundMoney < neededMoney, VarNote::error::WRONG_AMOUNT);
...
std::shared_ptr<WalletRequest> WalletTransactionSender::makeGetRandomOutsRequest(std::shared_ptr<SendTransactionContext> context) {
  ...
  for (auto idx: context->selectedTransfers) {
    ...
    throwIf(td.tx.vout.size() <= td.internalOutputIndex, VarNote::error::INTERNAL_WALLET_ERROR);
...
void WalletTransactionSender::sendTransactionRandomOutsByAmount(std::shared_ptr<SendTransactionContext> context, std::deque<std::shared_ptr<WalletEvent> >& events,
  ...
  if (m_isStoping) {
    events.push_back(std::make_shared<WalletSendTransactionCompletedEvent>(context->transactionId, make_error_code(VarNote::error::TX_CANCELLED)));
    return;
  }
  ...
  auto scanty_it = std::find_if(context->outs.begin(), context->outs.end(), [&] (VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount& out) {return out.outs.size() < context->mixIn;});

  if (scanty_it != context->outs.end()) {
    events.push_back(std::make_shared<WalletSendTransactionCompletedEvent>(context->transactionId, make_error_code(VarNote::error::MIXIN_COUNT_TOO_BIG)));
    return;
  }
...
std::shared_ptr<WalletRequest> WalletTransactionSender::doSendTransaction(std::shared_ptr<SendTransactionContext> context, std::deque<std::shared_ptr<WalletEvent> >& events) {
  ...
  try
  {
    ...
    VarNote::tx_destination_entry changeDts = AUTO_VAL_INIT(changeDts);
    createChangeDestinations(m_keys.m_account_address, transaction.totalAmount, context->foundMoney, changeDts);

    std::vector<VarNote::tx_destination_entry> splittedDests;
    ...
    events.push_back(std::make_shared<WalletSendTransactionCompletedEvent>(context->transactionId, make_error_code(VarNote::error::INTERNAL_WALLET_ERROR)));
  }
...
void WalletTransactionSender::splitDestinations(TransferId firstTransferId, size_t transfersCount, const VarNote::tx_destination_entry& changeDts,
                                                const TxDustPolicy& dustPolicy, std::vector<VarNote::tx_destination_entry>& splittedDests) {
  uint64_t dust = 0;

  digitSplitStrategy(firstTransferId, transfersCount, changeDts, dustPolicy.dustThreshold, splittedDests, dust);

  throwIf(dustPolicy.dustThreshold < dust, VarNote::error::INTERNAL_WALLET_ERROR);
  if (0 != dust && !dustPolicy.addToFee) {
    splittedDests.push_back(VarNote::tx_destination_entry(dust, dustPolicy.addrForDust));
  }
}


void WalletTransactionSender::digitSplitStrategy(TransferId firstTransferId, size_t transfersCount,
  const VarNote::tx_destination_entry& change_dst, uint64_t dust_threshold,
  std::vector<VarNote::tx_destination_entry>& splitted_dsts, uint64_t& dust) {
  splitted_dsts.clear();
  dust = 0;
  ...
  for (TransferId idx = firstTransferId; idx < firstTransferId + transfersCount; ++idx) {
    ...
    VarNote::account_public_address addr;
    if (!VarNote::get_account_address_from_str(addr, de.address)) {
      throw std::system_error(make_error_code(VarNote::error::BAD_ADDRESS));
    }

    VarNote::decompose_amount_into_digits(de.amount, dust_threshold,
      [&](uint64_t chunk) { splitted_dsts.push_back(VarNote::tx_destination_entry(chunk, addr)); },
      [&](uint64_t a_dust) { splitted_dsts.push_back(VarNote::tx_destination_entry(a_dust, addr)); } );
  }

  VarNote::decompose_amount_into_digits(change_dst.amount, dust_threshold,
    [&](uint64_t chunk) { splitted_dsts.push_back(VarNote::tx_destination_entry(chunk, change_dst.addr)); },
    [&](uint64_t a_dust) { dust = a_dust; } );
}


void WalletTransactionSender::prepareInputs(const std::list<size_t>& selectedTransfers, std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& outs,
    std::vector<VarNote::tx_source_entry>& sources, uint64_t mixIn) {
  size_t i = 0;
  for (size_t idx: selectedTransfers) {
    sources.resize(sources.size()+1);
    VarNote::tx_source_entry& src = sources.back();
    TransferDetails& td = m_transferDetails.getTransferDetails(idx);
    src.amount = td.amount();

    //paste mixin transaction
    if(outs.size()) {
      outs[i].outs.sort([](const VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry& a, const VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry& b){return a.global_amount_index < b.global_amount_index;});
      for (auto& daemon_oe: outs[i].outs) {
        if(td.globalOutputIndex == daemon_oe.global_amount_index)
          continue;
        VarNote::tx_source_entry::output_entry oe;
    ...
    auto it_to_insert = std::find_if(src.outputs.begin(), src.outputs.end(), [&](const VarNote::tx_source_entry::output_entry& a) { return a.first >= td.globalOutputIndex; });

    VarNote::tx_source_entry::output_entry real_oe;
    real_oe.first = td.globalOutputIndex;
    real_oe.second = boost::get<VarNote::txout_to_key>(td.tx.vout[td.internalOutputIndex].target).key;

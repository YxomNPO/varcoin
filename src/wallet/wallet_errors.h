...
namespace tools
{
  namespace error
  {
    ...
    {
      explicit unexpected_txin_type(std::string&& loc, const VarNote::transaction& tx)
      ...
      const VarNote::transaction& tx() const { return m_tx; }

      std::string to_string() const
      {
        std::ostringstream ss;
        VarNote::transaction tx = m_tx;
        ss << wallet_internal_error::to_string() << ", tx:\n" << VarNote::obj_to_json_str(tx);
        return ss.str();
      }
    ...
    {
      explicit acc_outs_lookup_error(std::string&& loc, const VarNote::transaction& tx,
        const crypto::public_key& tx_pub_key, const VarNote::account_keys& acc_keys)
      ...
      const VarNote::account_keys& acc_keys() const { return m_acc_keys; }

      std::string to_string() const
      {
        std::ostringstream ss;
        VarNote::transaction tx = m_tx;
        ss << refresh_error::to_string() << ", tx: " << VarNote::obj_to_json_str(tx);
        return ss.str();
      }

    private:
      const VarNote::transaction m_tx;
      const crypto::public_key m_tx_pub_key;
      const VarNote::account_keys m_acc_keys;
    };
    //----------------------------------------------------------------------------------------------------
    struct block_parse_error : public refresh_error
    {
      explicit block_parse_error(std::string&& loc, const VarNote::blobdata& block_data)
      ...
      const VarNote::blobdata& block_blob() const { return m_block_blob; }

      std::string to_string() const { return refresh_error::to_string(); }

    private:
      VarNote::blobdata m_block_blob;
    };
    //----------------------------------------------------------------------------------------------------
    ...
    {
      explicit tx_parse_error(std::string&& loc, const VarNote::blobdata& tx_blob)
      ...
      const VarNote::blobdata& tx_blob() const { return m_tx_blob; }

      std::string to_string() const { return refresh_error::to_string(); }

    private:
      VarNote::blobdata m_tx_blob;
    };
    //----------------------------------------------------------------------------------------------------
    ...
    struct not_enough_money : public transfer_error
    {
      ...
      std::string to_string() const
      {
        ...
        ss << transfer_error::to_string() <<
          ", available = " << VarNote::print_money(m_available) <<
          ", tx_amount = " << VarNote::print_money(m_tx_amount) <<
          ", fee = " << VarNote::print_money(m_fee);
        return ss.str();
      }
    ...
    {
      typedef std::vector<VarNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> scanty_outs_t;
      ...
      std::string to_string() const
      {
        ...
        {
          ss << '\n' << VarNote::print_money(outs_for_amount.amount) << " - " << outs_for_amount.outs.size();
        }
        return ss.str();
      }
    ...
    {
      typedef std::vector<VarNote::tx_source_entry> sources_t;
      typedef std::vector<VarNote::tx_destination_entry> destinations_t;
      ...
      std::string to_string() const
      {
        ...
        {
          const VarNote::tx_source_entry& src = m_sources[i];
          ss << "\n  source " << i << ":";
          ss << "\n    amount: " << VarNote::print_money(src.amount);
          ...
          //{
          //  const VarNote::tx_source_entry::output_entry& out = src.outputs[j];
        ...
        {
          const VarNote::tx_destination_entry& dst = m_destinations[i];
          ss << "\n  " << i << ": " << VarNote::get_account_address_as_str(dst.addr) << " " <<
            VarNote::print_money(dst.amount);
        }
    ...
    {
      explicit tx_rejected(std::string&& loc, const VarNote::transaction& tx, const std::string& status)
    ...
    {
      explicit tx_sum_overflow(std::string&& loc, const std::vector<VarNote::tx_destination_entry>& destinations, uint64_t fee)
        : transfer_error(std::move(loc), "transaction sum + fee exceeds " + VarNote::print_money(std::numeric_limits<uint64_t>::max()))
        , m_destinations(destinations)
        , m_fee(fee)
      {
      }
      ...
      const std::vector<VarNote::tx_destination_entry>& destinations() const { return m_destinations; }
      uint64_t fee() const { return m_fee; }
      ...
      std::string to_string() const
      {
        ...
        {
          ss << '\n' << VarNote::print_money(dst.amount) << " -> " << VarNote::get_account_address_as_str(dst.addr);
        }
        return ss.str();
      }

    private:
      std::vector<VarNote::tx_destination_entry> m_destinations;
      uint64_t m_fee;
    };
    //----------------------------------------------------------------------------------------------------
    struct tx_too_big : public transfer_error
    {
      explicit tx_too_big(std::string&& loc, const VarNote::transaction& tx, uint64_t tx_size_limit)

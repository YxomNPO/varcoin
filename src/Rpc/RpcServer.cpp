...
RpcServer::RpcServer(System::Dispatcher& dispatcher, Logging::ILogger& log, Core& c, NodeServer& p2p, IVarNoteProtocolHandler& protocol) :
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  ...
  VarNote::BinaryArray blob_reserve;
  ...
  PublicKey tx_pub_key = VarNote::getTransactionPublicKeyFromExtra(blockTemplate.baseTransaction.extra);

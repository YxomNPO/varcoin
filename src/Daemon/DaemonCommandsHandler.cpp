...
static bool print_as_json(const T& obj) {
  std::cout << VarNote::storeToJson(obj) << ENDL;
  return true;
}

std::string printTransactionShortInfo(const VarNote::CachedTransaction& transaction) {
  std::stringstream ss;
...
std::string printTransactionFullInfo(const VarNote::CachedTransaction& transaction) {
  std::stringstream ss;
  ss << printTransactionShortInfo(transaction);
  ss << "JSON: \n" << VarNote::storeToJson(transaction.getTransaction()) << std::endl;
...
DaemonCommandsHandler::DaemonCommandsHandler(VarNote::Core& core, VarNote::NodeServer& srv, Logging::LoggerManager& log) :
  m_core(core), m_srv(srv), logger(log, "daemon"), m_logManager(log) {
  m_consoleHandler.setHandler("exit", boost::bind(&DaemonCommandsHandler::exit, this, _1), "Shutdown the daemon =(e)");
  m_consoleHandler.setHandler("help", boost::bind(&DaemonCommandsHandler::help, this, _1), "Show this help =(h)");
  m_consoleHandler.setHandler("print_pl", boost::bind(&DaemonCommandsHandler::print_pl, this, _1), "Print peer list =(pp)");
  m_consoleHandler.setHandler("print_cn", boost::bind(&DaemonCommandsHandler::print_cn, this, _1), "Print connections =(pc)");
  m_consoleHandler.setHandler("print_bc", boost::bind(&DaemonCommandsHandler::print_bc, this, _1), "Print blockchain info in a given blocks range, print_bc <begin_height> [<end_height>] =(pbc)");
  ...
  m_consoleHandler.setHandler("print_block", boost::bind(&DaemonCommandsHandler::print_block, this, _1), "Print block, print_block <block_hash> | <block_height> =(pbk)");
  m_consoleHandler.setHandler("print_tx", boost::bind(&DaemonCommandsHandler::print_tx, this, _1), "Print transaction, print_tx <transaction_hash> =(pt)");
  m_consoleHandler.setHandler("print_pool", boost::bind(&DaemonCommandsHandler::print_pool, this, _1), "Print transaction pool (long format) =(ptp)");
  m_consoleHandler.setHandler("print_pool_sh", boost::bind(&DaemonCommandsHandler::print_pool_sh, this, _1), "Print transaction pool (short format) =(ptps)");
  m_consoleHandler.setHandler("set_log", boost::bind(&DaemonCommandsHandler::set_log, this, _1), "set_log <level> - Change current log level, <level> is a number 0-4 =(sl)");
  m_consoleHandler.setHandler("e", boost::bind(&DaemonCommandsHandler::exit, this, _1), "");
  m_consoleHandler.setHandler("h", boost::bind(&DaemonCommandsHandler::help, this, _1), "");
  m_consoleHandler.setHandler("pp", boost::bind(&DaemonCommandsHandler::print_pl, this, _1), "");
  m_consoleHandler.setHandler("pc", boost::bind(&DaemonCommandsHandler::print_cn, this, _1), "");
  m_consoleHandler.setHandler("pbc", boost::bind(&DaemonCommandsHandler::print_bc, this, _1), "");
  m_consoleHandler.setHandler("pbk", boost::bind(&DaemonCommandsHandler::print_block, this, _1), "");
  m_consoleHandler.setHandler("pt", boost::bind(&DaemonCommandsHandler::print_tx, this, _1), ">");
  m_consoleHandler.setHandler("ptp", boost::bind(&DaemonCommandsHandler::print_pool, this, _1), "");
  m_consoleHandler.setHandler("ptps", boost::bind(&DaemonCommandsHandler::print_pool_sh, this, _1), "");
  m_consoleHandler.setHandler("sl", boost::bind(&DaemonCommandsHandler::set_log, this, _1), "");
...
{
  std::stringstream ss;
  ss << VarNote::VARNOTE_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::print_tx(const std::vector<std::string>& args)
{
  ...
  std::vector<VarNote::BinaryArray> txs;
  std::vector<Crypto::Hash> missed_ids;
  m_core.getTransactions(tx_ids, txs, missed_ids);

  if (1 == txs.size()) {
    VarNote::CachedTransaction tx(txs.front());
...
bool DaemonCommandsHandler::print_pool(const std::vector<std::string>& args)
{
  ...
  for (const auto& tx: pool) {
    VarNote::CachedTransaction ctx(tx);
